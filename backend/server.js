...
const adsRoutes = (await import('./src/routes/ads.routes.js')).default;
...
const notificationsV1Routes = (await import('./src/routes/api/v1/notifications.routes.js')).default;
const adminReportsRoutes = (await import('./src/routes/adminReports.routes.js')).default;
const adminBranchesRoutes = (await import('./src/routes/adminBranches.routes.js')).default;
...
const adminNotificationsRoutes = (await import('./src/routes/adminNotifications.routes.js')).default;
...
const envOrigins = (process.env.ALLOWED_ORIGINS || '')
  ...
  .map(s => s.trim())
  .filter(Boolean);
const allowedOriginRegexRaw = String(process.env.ALLOWED_ORIGIN_REGEX || '').trim();
let allowedOriginRegex = null;
if (allowedOriginRegexRaw) {
  try {
    allowedOriginRegex = new RegExp(allowedOriginRegexRaw);
  } catch (_e) {
...
const corsOptions = {
  origin: (origin, callback) => {
    ...
    if (allowedOriginRegex && allowedOriginRegex.test(origin)) return callback(null, true);
...
console.log('✅ CORS enabled for:', allowedOrigins, allowedOriginRegex ? `regex=${allowedOriginRegex}` : '');
...
app.use(gated('json', express.json({ limit: process.env.JSON_BODY_LIMIT || '5mb' }), true));
app.use(gated('urlencoded', express.urlencoded({ extended: true, limit: process.env.URLENCODED_BODY_LIMIT || '5mb' }), true));
...
if (process.env.NODE_ENV === 'production' && ALLOWED_HOSTS.length) {
  app.use((req, res, next) => {
    ...
    const hostHeaderRaw = forwardedHostRaw || String(req.headers.host || '').trim();
    const hostLower = hostHeaderRaw.toLowerCase();
    const hostnameOnly = hostLower.split(',')[0].trim().split(':')[0].trim();
    const isAllowed = ALLOWED_HOSTS.includes(hostLower) || ALLOWED_HOSTS.includes(hostnameOnly);
    if (!isAllowed) {
...
const routes = [
  ...
  { path: '/api/v1/notifications', handler: notificationsV1Routes },
  ...
  { path: '/api/ads', handler: adsRoutes },
  { path: '/api/admin/config', handler: adminConfigRoutes },
  { path: '/api/admin/users', handler: adminUsersRoutes },
  { path: '/api/admin/branches', handler: adminBranchesRoutes },
  { path: '/api/admin/reports', handler: adminReportsRoutes },
  { path: '/api/admin/notifications', handler: adminNotificationsRoutes },
...
if (String(process.env.ODOO_BUS_LISTENER_ENABLED || '').trim().toLowerCase() === 'true') {
  startOdooListener();
}
