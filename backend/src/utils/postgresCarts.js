...
const extractCartStatus = (cart = {}) => {
  ...
  const direct = cart.status;
  if (typeof direct === 'string' && direct.trim()) return direct.trim();

  const bb = cart.busbudResponse && typeof cart.busbudResponse === 'object' ? cart.busbudResponse : null;
  if (bb) {
    const s1 = bb.status;
    if (typeof s1 === 'string' && s1.trim()) return s1.trim();

    const raw = bb._raw && typeof bb._raw === 'object' ? bb._raw : null;
    const s2 = raw && raw.status;
    if (typeof s2 === 'string' && s2.trim()) return s2.trim();

    const cartObj = bb.cart && typeof bb.cart === 'object' ? bb.cart : null;
    const s3 = cartObj && cartObj.status;
    if (typeof s3 === 'string' && s3.trim()) return s3.trim();
  }

  const md = cart.metadata && typeof cart.metadata === 'object' ? cart.metadata : null;
  const s4 = md && (md.status || md.cartStatus);
  if (typeof s4 === 'string' && s4.trim()) return s4.trim();
...
export async function upsertCartFromFirestore(cartId, updates = {}) {
  try {
    ...
    const canonicalCartId = String(
      cart.firestoreCartId ||
        cart.id ||
        cart.cartId ||
        cart.cart_id ||
        cartId ||
    ...
    const record = {
      ...
      firestoreCartId: cart.firestoreCartId || cart.id || String(cartId),
      bookedBy,
      status: status || null,
      currency,
      origin,
...
export async function upsertCartPurchaserFromBusbud(cartId, purchaser = {}) {
  try {
    ...
    const canonicalCartId = await (async () => {
      try {
        const rows = await drizzleDb
        ...
        return resolved ? String(resolved).trim() : busbudCartId;
      } catch (_) {
        return busbudCartId;
      }
    })();
    ...
    await drizzleDb
      ...
      .values({
        cartId: canonicalCartId,
        firestoreCartId: canonicalCartId,
    ...
    try {
      await mirrorCartToFirestoreFromPostgres(canonicalCartId, null);
    } catch (_) {
      // ignore
    }
  } catch (e) {
