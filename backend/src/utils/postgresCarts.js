...
const ensureCartsTableExists = async () => {
  ...
  await drizzleDb.execute(sql`ALTER TABLE "carts" DROP COLUMN IF EXISTS "firestore_cart_id";`);
...
export async function upsertCartFromFirestore(cartId, updates = {}) {
  try {
    ...
    const pnrCartId = (() => {
      const fromCart = cart && (cart.pnr || cart.pnrId || cart.pnr_id || cart.firestoreCartId || cart.reference || cart.referenceId);
      if (fromCart && String(fromCart).trim()) return String(fromCart).trim();
      if (looksLikePnr(cartId)) return String(cartId).trim();
      const fromExtra = updates && (updates.pnr || updates.firestoreCartId || updates.reference);
      if (fromExtra && String(fromExtra).trim()) return String(fromExtra).trim();
      return null;
    })();

    const busbudCartId = (() => {
      const cand = cart && (cart.busbudCartId || cart.busbud_cart_id || cart.cartId || cart.cart_id);
      if (cand && String(cand).trim()) return String(cand).trim();
      if (!looksLikePnr(cartId)) return String(cartId).trim();
      return null;
    })();

    // Guard: never write a Postgres carts row keyed by the Busbud cart id.
    if (!pnrCartId) return;
    ...
    const record = {
      cartId: String(pnrCartId),
      busbudCartId: busbudCartId ? String(busbudCartId) : null,
      bookedBy,
      status: status || null,
      currency,
      origin,
    ...
    const updateSet = {
      busbudCartId: record.busbudCartId,
...
export async function mirrorCartToFirestoreFromPostgres(cartId, firestoreCartId = null) {
  try {
    ...
    const db = await ensureFirestoreDb();
    const docId = String(firestoreCartId || pgCart.cartId || cartId);
    const nowIso = new Date().toISOString();

    const payload = {
      busbudCartId: String(pgCart.busbudCartId || ''),
...
export async function upsertCartFromBusbud(cartId, busbudResponse = {}, extra = {}) {
  ...
  const pnrCartId = (() => {
    const fromExtra = extra && (extra.pnr || extra.firestoreCartId || extra.reference || extra.cartId);
    if (fromExtra && String(fromExtra).trim()) return String(fromExtra).trim();
    return null;
  })();

  // Guard: never upsert by Busbud id.
  if (!pnrCartId || !looksLikePnr(pnrCartId)) {
    return;
  }
  ...
  await upsertCartFromFirestore(String(pnrCartId), cart, { skipFirestoreRead: true });
  try {
    await mirrorCartToFirestoreFromPostgres(String(pnrCartId), String(pnrCartId));
  } catch (_) {
    // ignore
  }
}
...
export async function upsertCartPurchaserFromBusbud(cartId, purchaser = {}) {
  try {
    ...
    const resolvedPnrCartId = await (async () => {
      try {
        const rows = await drizzleDb
    ...
    if (!resolvedPnrCartId) return;

    await drizzleDb
      .insert(cartsTable)
      .values({
        cartId: String(resolvedPnrCartId),
        busbudCartId,
    ...
    try {
      await mirrorCartToFirestoreFromPostgres(String(resolvedPnrCartId), String(resolvedPnrCartId));
    } catch (_) {
      // ignore
    }
  } catch (e) {
  ...
  try {
    ...
    const rows = await drizzleDb
      .select({
        ...
        busbudCartId: cartsTable.busbudCartId,
      ...
      .from(cartsTable)
      .where(or(eq(cartsTable.cartId, id), eq(cartsTable.busbudCartId, id)))
      .limit(1);
