...
function resolvePuppeteerExecutablePath() {
  ...
  try {
    ...
    if (envPath && String(envPath).trim() && exists(envPath)) return String(envPath).trim();
  } catch (_) {}

  // Common Linux paths (Ubuntu/Debian/Alpine variants)
  const candidates = [
  ...
  try {
    ...
    if (p && String(p).trim() && exists(p)) return String(p).trim();
  } catch (_) {}
