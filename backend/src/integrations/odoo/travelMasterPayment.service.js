...
class TravelMasterAPI {
  constructor(config = {}) {
    ...
    const methods = [
      'authenticate', 'ensureAuthenticated', 'executeMethod', 'findOrCreatePartner', 'createInvoice',
      ...
      'updateBookingStatus', 'getDefaultInboundJournalId', 'registerPayment'
    ];
  ...
  async executeMethod(model, method, args = [], kwargs = {}) {
    ...
    const call = async () => {
      ...
      return this.modelsClient.methodCall('execute_kw', params);
    };

    try {
      return await call();
    } catch (error) {
      const msg = String(error?.faultString || error?.message || '');
      const shouldRetry = /not authenticated|accessdenied|access denied|session/i.test(msg);
      if (shouldRetry) {
        this.uid = null;
        await this.ensureAuthenticated();
        if (this.uid) {
          try {
            ...
            console.error(`❌ API call failed (${model}.${method}):`, e2);
            throw e2;
          }
        }
      }
  ...
  // Find a default inbound journal (bank or cash)
  async getDefaultInboundJournalId() {
    const journalIds = await this.executeMethod('account.journal', 'search', [[['type', 'in', ['bank', 'cash']]]], { limit: 1 });
    if (Array.isArray(journalIds) && journalIds.length) return journalIds[0];
    throw new Error('No inbound (bank/cash) journal found in Odoo');
  }
  ...
  async searchByPNR(pnr) {
    try {
      ...
      const fields = [
        'id', 'name', 'payment_reference', 'invoice_date',
  ...
  // ──────────────────────────────── REGISTER PAYMENT ────────────────────────────────
  async registerPayment(invoiceId, options = {}) {
    // Ensure invoice is valid and posted
    const [inv] = await this.readInvoices([invoiceId], [
    ...
    if (!amount || amount <= 0) {
      throw new Error(`Invalid payment amount for invoice ${invoiceId}`);
    }

    const journalId = options.journal_id || options.journalId || await this.getDefaultInboundJournalId();
    const paymentDate = options.payment_date || options.paymentDate || new Date().toISOString().slice(0, 10);
    const communication = options.communication || options.memo || `Payment registered via API for ${inv.name || invoiceId}`;
    ...
    let wizardId;
    try {
      wizardId = await this.executeMethod(
      ...
      logger.error('[Odoo] Failed to create account.payment.register wizard', { invoiceId, amount, journalId, paymentDate, error: msg });
      throw new Error('Could not initialize Odoo payment wizard. Please try again or contact support.');
    }

    // Some Odoo setups/libraries may return [id] instead of id
    if (Array.isArray(wizardId)) wizardId = wizardId[0];
    const wizardNumericId = Number(wizardId);
    if (!wizardNumericId || !Number.isFinite(wizardNumericId)) {
      console.error('[registerPayment] Invalid wizard id returned:', wizardId);
      throw new Error('Failed to create payment register wizard');
    }
    logger.info('[Odoo] Payment wizard created', { wizardId: wizardNumericId, invoiceId, amount, journalId });

    // Create payment from wizard
    try {
      await this.executeMethod('account.payment.register', 'action_create_payments', [[wizardNumericId]], { context: ctx });
    } catch (err) {
      const raw = err?.message || String(err);
      logger.error('[Odoo] action_create_payments failed', { wizardId: wizardNumericId, invoiceId, error: raw, ctx });
      let friendly = 'Odoo failed to create the payment. Ensure the invoice is posted and not already paid.';
      if (raw && raw.toLowerCase().includes('unhashable type')) {
        friendly = 'Odoo payment wizard failed due to invalid context. Please try again in a moment.';
      }
      throw new Error(friendly);
    }

    return { success: true, invoiceId, amount, journalId, paymentDate };
  }
