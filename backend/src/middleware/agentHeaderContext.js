...
export default async function agentHeaderContext(req, _res, next) {
  try {
    ...
    // If agent has an assigned branch, stamp x-client-branch with its numeric code
    try {
      const branchId = agent && agent.branchId ? String(agent.branchId).trim() : '';
      if (branchId) {
        const rows = await drizzleDb.select().from(branchesTable).where(eq(branchesTable.id, branchId)).limit(1);
        const branch = rows && rows.length ? rows[0] : null;
        const code = branch && branch.active !== false ? String(branch.code || '').trim() : '';
        if (code && /^\d{2}$/.test(code)) {
          req.headers = req.headers || {};
          if (!req.headers['x-client-branch']) req.headers['x-client-branch'] = code;
        }
      }
    } catch (_) {
    }
