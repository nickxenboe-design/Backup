...
async function fetchAgentRecentBookings(agentEmail, limit, extras = {}) {
    ...
    try {
      ...
      if (refs.length) {
        const cartRows = await drizzleDb
          ...
          .from(cartsPgTable)
          .where(or(inArray(cartsPgTable.cartId, refs), inArray(cartsPgTable.firestoreCartId, refs)));
  ...
  try {
    ...
    // If older rows are missing bookedBy, fall back to Firestore cart agentEmail/bookedBy
    // and backfill Postgres so subsequent requests work without extra reads.
    const fsBookedByByRef = new Map();
    try {
      const candidateRefs = [];
      const seenCandidate = new Set();

      for (const r of cartRows || []) {
        const statusRaw = r && r.status ? r.status : '';
        if (!isAwaitingPaymentOrFailedStatus(statusRaw)) continue;
        if (r && r.bookedBy) continue;

        const rawCartId = String(r && r.cartId ? r.cartId : '').trim();
        const rawFirestoreId = String(r && r.firestoreCartId ? r.firestoreCartId : '').trim();
        const ref = rawFirestoreId || rawCartId;
        if (!ref) continue;
        if (seenCandidate.has(ref)) continue;
        seenCandidate.add(ref);
      ...
      if (candidateRefs.length) {
        const fs = await getFirestore();
        const docRefs = candidateRefs.map((id) => fs.collection('carts').doc(String(id)));
        const snaps = await fs.getAll(...docRefs);
        for (const snap of snaps || []) {
          if (!snap || !snap.exists) continue;
          const data = snap.data() || {};
          const ref = String(snap.id || '').trim();
          if (!ref) continue;
          const agentEmail = normalizeEmailLocal(data.agentEmail || (data.agent && data.agent.agentEmail) || '');
          const bookedByText = String(data.bookedBy || data.booked_by || data.agentName || (data.agent && data.agent.agentName) || '').trim();
          fsBookedByByRef.set(ref, {
            agentEmail,
            bookedByText,
            bookedByEmail: extractEmailFromTextLocal(bookedByText)
          });
        }
      }
    } catch (_) {
    ...
    for (const row of cartRows || []) {
      ...
      const bookedByNorm = normalizeEmailLocal(row.bookedBy || '');
      const bookedByEmail = extractEmailFromTextLocal(row.bookedBy || '');
      const fsMeta = fsBookedByByRef.get(canonicalRef) || null;
      const bookedByEmailFromFs = fsMeta && fsMeta.bookedByEmail ? fsMeta.bookedByEmail : '';
      const bookedByNormFromFs = fsMeta && fsMeta.agentEmail ? fsMeta.agentEmail : '';
      const matchesAgent =
        (bookedByEmail && bookedByEmail === normalizedAgent) ||
        (bookedByNorm && bookedByNorm === normalizedAgent) ||
        (bookedByEmailFromFs && bookedByEmailFromFs === normalizedAgent) ||
        (bookedByNormFromFs && bookedByNormFromFs === normalizedAgent);
      if (!matchesAgent) continue;

      if (!row.bookedBy && bookedByNormFromFs) {
        drizzleDb
          .update(cartsPgTable)
          .set({ bookedBy: bookedByNormFromFs })
          .where(eq(cartsPgTable.cartId, String(row.cartId)))
          .catch(() => {});
      }
      ...
      const buildRow = () => ({
        ...
        bookingType: normalizeStatusLocal(statusRaw) === 'failed' ? 'failed' : 'unpaid',
        source: 'carts_pg',
        passengers: passengersCount,
        revenue,
        currency,
...
async function buildAgentSalesSummary(agentEmail, query, extras = {}) {
  ...
  for (let page = 0; page < maxPages && !exhaustedRange; page++) {
    ...
    for (const row of paymentRows) {
      ...
      const pgBusbud = (pgCart && pgCart.busbudResponse && typeof pgCart.busbudResponse === 'object') ? pgCart.busbudResponse : null;
      const providerRaw = pgBusbud && (pgBusbud.provider || pgBusbud.Provider) ? String(pgBusbud.provider || pgBusbud.Provider).trim() : '';
      const provider = providerRaw ? providerRaw.toLowerCase() : '';
      const isEagleliner = provider === 'eagleliner';
      ...
      const revenue = Number(row.amount || 0);
      let currency = deriveCurrencyFromPurchase(purchase) || (pgCart && pgCart.currency) || 'USD';
      if (isEagleliner) currency = (pgCart && pgCart.currency) || 'ZAR';
      ...
      if (rows.length < limit) {
        rows.push({
          ...
          operator: isEagleliner ? 'Eagleliner' : null,
          origin: (pgCart && pgCart.origin) ? pgCart.origin : null,
          destination: (pgCart && pgCart.destination) ? pgCart.destination : null,
          branch: null,
          paymentType: row.method || null,
          status: row.status || null,
          passengers: passengersCount,
