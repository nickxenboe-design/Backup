...
async function savePurchaseToPostgres(purchaseId, purchaseUuid, purchaseDetails, extraMeta, pollOutcome, requestId) {
  try {
    ...
    if (exMode) {
      ...
      if (exName && String(exName).trim()) {
        bookedBy = String(exName).trim();
      } else if (exCombined) {
        bookedBy = exCombined;
      } else if (exEmail && String(exEmail).trim()) {
    ...
    // If still unknown, prefer the canonical Postgres carts.booked_by (PNR keyed)
    const looksLikePnrLocal = (v) => /^[0-9]{8,}$/.test(String(v || '').trim());
    let resolvedPnrFromPg = null;
    try {
      await ensureCartsColumnsExist();
      const candidates = [
        ex.paymentRef,
        ...
        booking && (booking.cartId || booking.cart_id),
        purchaseDetails && purchaseDetails.reference,
        purchaseDetails && (purchaseDetails.cartId || purchaseDetails.cart_id),
        purchaseId
      ]
      ...
      if (candidates.length) {
        const cartRows = await drizzleDb
          .select({ cartId: cartsPgTable.cartId, bookedBy: cartsPgTable.bookedBy })
          .from(cartsPgTable)
          .where(or(inArray(cartsPgTable.cartId, candidates), inArray(cartsPgTable.busbudCartId, candidates)))
          .limit(1);
        const cartRow = cartRows && cartRows.length ? (cartRows[0] || null) : null;
        const pgBookedBy = cartRow && cartRow.bookedBy ? String(cartRow.bookedBy).trim() : '';
        const pgCartId = cartRow && cartRow.cartId ? String(cartRow.cartId).trim() : '';
        if (pgCartId && looksLikePnrLocal(pgCartId)) resolvedPnrFromPg = pgCartId;
        if (!bookedBy && pgBookedBy) bookedBy = pgBookedBy;
      }
    } catch (_) {}
    ...
    try {
      if (!bookedBy) {
        ...
        for (const ref of fsCandidates) {
          if (!ref) continue;
          let snap = await db.collection('carts').doc(String(ref)).get();
          let cart = (snap && snap.exists) ? (snap.data() || {}) : null;

          if (!cart && ref && !looksLikePnrLocal(ref)) {
            try {
              const q = await db.collection('carts').where('busbudCartId', '==', String(ref)).limit(1).get();
              if (q && !q.empty) {
                const d = q.docs[0];
                snap = d;
                cart = d.data() || {};
          ...
          } else if (aEmail && String(aEmail).trim()) {
            bookedBy = String(aEmail).trim().toLowerCase();
          } else {
            bookedBy = 'agent';
          }

          if (!resolvedPnrFromPg && looksLikePnrLocal(ref)) resolvedPnrFromPg = String(ref).trim();

          try {
            if (resolvedPnrFromPg && bookedBy && bookedBy !== 'online') {
              const now = new Date();
              await drizzleDb
                .insert(cartsPgTable)
                .values({
                  cartId: String(resolvedPnrFromPg),
    ...
    let transactionRef =
      ...
      purchaseId;
    if (transactionRef && !looksLikePnrLocal(transactionRef) && resolvedPnrFromPg && looksLikePnrLocal(resolvedPnrFromPg)) {
      transactionRef = resolvedPnrFromPg;
    }
    ...
    await drizzleDb
      ...
      .onConflictDoUpdate({
        ...
        set: {
          ...
          status: 'paid',
          bookedBy: sql`CASE 
            WHEN (${payments.bookedBy} IS NULL OR ${payments.bookedBy} = '' OR ${payments.bookedBy} = 'online') AND ${bookedBy} <> 'online' THEN ${bookedBy}
            WHEN (${payments.bookedBy} IS NULL OR ${payments.bookedBy} = '') THEN ${bookedBy}
            ELSE ${payments.bookedBy}
          END`,
          rawResponse: purchaseDetails
        }
      });
