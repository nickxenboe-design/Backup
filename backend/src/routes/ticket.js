...
import { printTickets as printEaglelinerTickets } from '../services/eagleliner/reservations.service.js';
  ...
  try {
    ...
    for (let i = 0; i < 6; i += 1) {
      ...
      const candidates = [
        ...
        v.payload,
        v.result,
        v.response && v.response.data,
        v.response && v.response.body && v.response.body.data,
        v.body && v.body.data,
      ].filter(Boolean);
...
const detectTicketProvider = (cart) => {
  try {
    if (!cart || typeof cart !== 'object') return 'busbud';
    const direct = cart.provider || cart.source;
    if (direct) return String(direct).trim().toLowerCase();
    const br = cart.busbudResponse || (cart.passengerDetails && cart.passengerDetails.busbudResponse) || null;
    if (br && typeof br === 'object' && br.provider) return String(br.provider).trim().toLowerCase();
    return 'busbud';
  } catch (_) {
    return 'busbud';
  }
...
const applyEaglelinerPrintTicketsToCart = async (cart, { pnr, requestId }) => {
  const c = cart && typeof cart === 'object' ? cart : {};
  const br = c.busbudResponse && typeof c.busbudResponse === 'object' ? c.busbudResponse : null;
  const reservationId = String(
    c.eaglelinerReservationId ||
      (c.eagleliner && c.eagleliner.reservationId) ||
      (br && (br.eaglelinerReservationId || (br.eagleliner && br.eagleliner.reservationId))) ||
      ''
  ).trim();

  if (!reservationId) {
    const err = new Error('Eagleliner reservationId not found');
    err.statusCode = 404;
    throw err;
  }

  const data = await printEaglelinerTickets({ reservationId });
  if (!data || data.Success === false) {
    const msg = data && (data.Error || data.error || data.message) ? String(data.Error || data.error || data.message) : 'Eagleliner ticket printing failed';
  ...
  if (!tickets.length) {
    const err = new Error('No Eagleliner tickets returned');
    err.statusCode = 502;
    err.details = data || null;
    throw err;
  }

  const basePassengers = Array.isArray(c.passengers)
    ? c.passengers
    : (br && br.eagleliner && Array.isArray(br.eagleliner.passengerDetails) ? br.eagleliner.passengerDetails : []);

  const mappedPassengers = tickets.map((t, idx) => {
    ...
    const phone = t && (t.Telephone || t.phone || t.phoneNumber) ? String(t.Telephone || t.phone || t.phoneNumber) : (p.Telephone || p.phone || p.phoneNumber || '');
    ...
    return {
      ...
      phone: p.phone || p.phoneNumber || p.Telephone || phone,
      phoneNumber: p.phoneNumber || p.phone || p.Telephone || phone,
  ...
  c._eaglelinerTickets = tickets;
  c.passengers = mappedPassengers;
  c.requiredPassengers = mappedPassengers;
  c.passengerCount = mappedPassengers.length;
  if (!c.passengerDetails || typeof c.passengerDetails !== 'object') c.passengerDetails = {};
  c.passengerDetails.passengers = mappedPassengers;

  const firstTicket = tickets[0] || {};
  if (firstTicket.TicketNumber && !c.ticketNo) c.ticketNo = String(firstTicket.TicketNumber);
  if (!c.paymentMethod && !c.payment_method) c.paymentMethod = 'Cash';
  if (firstTicket.Operator && !c.operatorName) c.operatorName = String(firstTicket.Operator);
  if (firstTicket.DepartureStopName) c.originCity = String(firstTicket.DepartureStopName);
  if (firstTicket.DestinationStopName) c.destinationCity = String(firstTicket.DestinationStopName);
  if (firstTicket.DepartureDateTime) c.departAt = String(firstTicket.DepartureDateTime);
  if (firstTicket.DestinationArrivalTime) c.arriveAt = String(firstTicket.DestinationArrivalTime);

  const hasAnyTotal = (() => {
    const candidates = [c.totalAmount, c.totalPrice, c.total, c.retailPrice];
    return candidates.some((v) => {
      const n = typeof v === 'number' ? v : (typeof v === 'string' ? parseFloat(String(v).match(/[0-9]+(?:\.[0-9]+)?/)?.[0] || 'NaN') : NaN);
  ...
  if (!hasAnyTotal) {
    const estimated = (() => {
      try {
        const fromCart = c.eagleliner && typeof c.eagleliner === 'object' ? c.eagleliner.estimatedTotal : null;
        const fromProvider = br && br.eagleliner && typeof br.eagleliner === 'object' ? br.eagleliner.estimatedTotal : null;
        const fromTicket = firstTicket && firstTicket.Amount != null ? firstTicket.Amount : null;
        const raw = fromCart ?? fromProvider ?? fromTicket;
        const n = typeof raw === 'number' ? raw : (typeof raw === 'string' ? parseFloat(String(raw).match(/[0-9]+(?:\.[0-9]+)?/)?.[0] || 'NaN') : NaN);
    ...
    if (estimated != null) {
      c.totalPrice = estimated;
      c.totalAmount = estimated;
      c.total = estimated;
    }
  }

  if (!c._ticketRenderFlags || typeof c._ticketRenderFlags !== 'object') {
    c._ticketRenderFlags = { provider: 'eagleliner' };
  }

  try {
    const pnrKey = String(pnr || c.cartId || c.firestoreCartId || '').trim();
    if (pnrKey) {
      const originName = firstTicket.DepartureStopName ? String(firstTicket.DepartureStopName) : null;
      const destinationName = firstTicket.DestinationStopName ? String(firstTicket.DestinationStopName) : null;
      const departAt = firstTicket.DepartureDateTime ? String(firstTicket.DepartureDateTime) : (c.departAt ? String(c.departAt) : null);
      const arriveAt = firstTicket.DestinationArrivalTime ? String(firstTicket.DestinationArrivalTime) : (c.arriveAt ? String(c.arriveAt) : null);

      const existingProvider = c.busbudResponse && typeof c.busbudResponse === 'object' ? c.busbudResponse : null;
      const nowMs = Date.now();
      let nextProvider = existingProvider && typeof existingProvider === 'object'
        ? { ...existingProvider }
        : {
          provider: 'eagleliner',
          source: 'eagleliner',
          eaglelinerReservationId: reservationId,
          eagleliner: {
            reservationId,
            tickets,
            printedAtMs: nowMs,
      ...
      if (!nextProvider.eaglelinerReservationId) nextProvider.eaglelinerReservationId = reservationId;
      ...
      nextProvider.eagleliner = {
        ...existingEagle,
        reservationId: existingEagle.reservationId || reservationId,
        tickets,
        printedAtMs: nowMs,
      };
      ...
      await drizzleDb
        .update(cartsPgTable)
        .set({
          origin: originName || (c.originCity ? String(c.originCity) : (c.origin ? String(c.origin) : null)),
          destination: destinationName || (c.destinationCity ? String(c.destinationCity) : (c.destination ? String(c.destination) : null)),
          departAt,
          arriveAt,
          passengers: mappedPassengers,
          passengerCount: mappedPassengers.length,
          busbudResponse: nextProvider,
          updatedAt: new Date(),
        })
        .where(or(eq(cartsPgTable.cartId, pnrKey), eq(cartsPgTable.firestoreCartId, pnrKey)));
    }
  } catch (e) {
    logger.warn(`[${requestId}] Failed to persist Eagleliner print_tickets data to Postgres`, {
      pnr: String(pnr || ''),
      reservationId,
  ...
  logger.info(`[${requestId}] Hydrated Eagleliner tickets for PDF`, { pnr: String(pnr), reservationId, tickets: tickets.length });
  return c;
};
...
const useFirestoreTicketFallback = process.env.ETICKET_USE_FIRESTORE_FALLBACK === 'true';
              ...
              try {
                ...
                if (purchaseId && purchaseUuid) {
                  ...
                  if (fetched && typeof fetched === 'object') {
                    ...
                    drizzleDb
                      .update(payments)
                      .set({ rawResponse: fetchedNormalized })
                      .where(eq(payments.transactionRef, pnrId))
                      .catch(() => {});
                  } else {
...
router.post(
  ...
  try {
    ...
    const ticketNo = (() => {
      ...
      const cpRef = ticketNoForPassengerIndex(cp, 0, null);
...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const detectedProvider = detectTicketProvider(cart);
      res.setHeader('x-ticket-provider', detectedProvider);
      if (detectedProvider === 'eagleliner') {
        try {
          cart = await applyEaglelinerPrintTicketsToCart(cart, { pnr, requestId });
        } catch (e) {
          const status = e && e.statusCode ? Number(e.statusCode) : 502;
          logger.warn(`[${requestId}] Failed to hydrate Eagleliner tickets for PDF`, {
            pnr: String(pnr),
            error: e && e.message ? e.message : String(e),
            status
          });
          return res.status(Number.isFinite(status) ? status : 502).send(e && e.message ? e.message : 'Failed to fetch Eagleliner tickets');
        }
      }
      ...
      const ticketNoForPassengerAt = (idx) => {
        try {
          const eTickets = cart && Array.isArray(cart._eaglelinerTickets) ? cart._eaglelinerTickets : null;
          const t = eTickets && eTickets[idx] ? eTickets[idx] : null;
          const tn = t && (t.TicketNumber || t.ticketNumber || t.ticket_no || t.ticketNo) ? (t.TicketNumber || t.ticketNumber || t.ticket_no || t.ticketNo) : null;
          if (tn) return String(tn);
        } catch (_) {}
        return ticketNoForPassengerIndex(completePurchase, idx, ticketNo);
      };
...
async function unifiedTicketPdfHandler(req, res) {
  ...
  try {
    ...
    if (!v.isEmpty()) {
      ...
      if (explicitType === 'final' || explicitType === 'hold') {
        res.setHeader('x-ticket-type-requested', explicitType);
      }

      const strictType =
        (q.strict === '1' || q.strict === 1 || q.strict === true || q.strict === 'true') ||
        (q.enforce === '1' || q.enforce === 1 || q.enforce === true || q.enforce === 'true') ||
        (q.require === '1' || q.require === 1 || q.require === true || q.require === 'true');
      ...
      if (!forceRegen) {
        if (wantsZip) {
          const cachedZip = explicitType === 'hold' ? null : await getCachedTicketPdfFromPostgres(pnr, 'final_zip');
          if (cachedZip && cachedZip.length) {
            res.setHeader('x-ticket-cache', 'hit');
            res.setHeader('x-ticket-cache-kind', 'final_zip');
            res.setHeader('x-ticket-type-served', 'final');
        ...
        if (!isThermal) {
          const cachedHoldOnly = explicitType === 'final' ? null : await getCachedTicketPdfFromPostgres(pnr, 'hold');
          const cachedFinalOnly = explicitType === 'hold' ? null : await getCachedTicketPdfFromPostgres(pnr, 'final');
          if (explicitType !== 'final' && cachedHoldOnly && !cachedFinalOnly) {
            if (!looksLikePdfBuffer(cachedHoldOnly)) {
              ...
              res.setHeader('x-ticket-cache', 'hit');
              res.setHeader('x-ticket-cache-kind', 'hold');
              res.setHeader('x-ticket-type-served', 'hold');
          ...
          const cachedFinal = explicitType === 'hold' ? null : await getCachedTicketPdfFromPostgres(pnr, 'final');
          const cachedHold = (explicitType === 'final' || cachedFinal) ? null : await getCachedTicketPdfFromPostgres(pnr, 'hold');
          ...
          if (cached) {
            ...
            if (!looksLikePdfBuffer(cached)) {
              ...
              res.setHeader('x-ticket-cache', 'hit');
              res.setHeader('x-ticket-cache-kind', cacheKind);
              res.setHeader('x-ticket-type-served', cacheKind);
      ...
      let hasFinalSource = 'none';
      if (usePostgresFirstForEticket) {
        try {
          pgCartForStatus = await loadTicketCartFromPostgres(pnr, requestId);
          hasFinal = hasFinalFromCart(pgCartForStatus);
          hasFinalSource = 'postgres';
        } catch (_) {
          // ignore
        }
      }

      // Additional signal: if payments table shows a paid/completed transaction, treat as final.
      if (!hasFinal) {
        try {
          const payRows = await drizzleDb
            ...
            .where(eq(payments.transactionRef, String(pnr)))
            .limit(1);
          const payStatus = payRows && payRows.length ? payRows[0]?.status : null;
          const s = payStatus == null ? '' : String(payStatus).trim().toLowerCase();
          const paidLike = ['paid', 'complete', 'completed', 'success', 'succeeded', 'confirmed', 'booked'].includes(s);
          if (paidLike) {
            hasFinal = true;
            hasFinalSource = 'payments';
          }
        } catch (_) {
          // ignore
        }
      ...
      const detectedHasFinal = hasFinal;

      if (explicitType === 'final') hasFinal = true;
      if (explicitType === 'hold') hasFinal = false;

      res.setHeader('x-ticket-has-final-detected', detectedHasFinal ? '1' : '0');
      res.setHeader('x-ticket-has-final-source', hasFinalSource);
      res.setHeader('x-ticket-has-final-effective', hasFinal ? '1' : '0');
      res.setHeader('x-ticket-strict', strictType ? '1' : '0');
      res.setHeader('x-ticket-primary-kind', hasFinal ? 'final' : 'hold');
      ...
      function normalizeUpstreamFailureStatus(status) {
      ...
      res.setHeader('x-ticket-upstream-primary-status', String(pdfResponse && pdfResponse.status != null ? pdfResponse.status : ''));

      // If a type was explicitly requested in strict mode, never fall back to the other type.
      if ((explicitType === 'final' || explicitType === 'hold') && strictType && pdfResponse.status !== 200) {
        logger.warn(`[${requestId}] Strict ticket type request failed`, { pnr, status: pdfResponse.status, requested: explicitType, url: primaryUrl });
        const outStatus = normalizeUpstreamFailureStatus(pdfResponse.status);
        const wanted = explicitType === 'final' ? 'final e-ticket' : 'reserved ticket';
        return res.status(outStatus).send(`Failed to generate ${wanted} (upstream status ${pdfResponse.status})`);
      }
      ...
      if (!hasFinal && pdfResponse.status !== 200) {
        ...
        res.setHeader('x-ticket-upstream-fallback-status', String(fallback && fallback.status != null ? fallback.status : ''));
      ...
      if (hasFinal && pdfResponse.status !== 200) {
        if (!strictType) {
          try {
            const cachedHoldFallback = await getCachedTicketPdfFromPostgres(pnr, 'hold');
            if (cachedHoldFallback && looksLikePdfBuffer(cachedHoldFallback)) {
              res.setHeader('x-ticket-cache', 'hit');
              res.setHeader('x-ticket-cache-kind', 'hold');
              res.setHeader('x-ticket-type-served', 'hold');
              const filename = `reserved-ticket-${encodeURIComponent(String(pnr))}.pdf`;
              ...
              res.setHeader('Content-Type', 'application/pdf');
              res.setHeader('Content-Disposition', `inline; filename="${filename}"`);
              const h = sha256Hex(cachedHoldFallback);
              if (h) res.setHeader('x-ticket-pdf-sha256', h);
              res.setHeader('Content-Length', cachedHoldFallback.length);
              return res.end(cachedHoldFallback);
            }
          } catch (_) {
            // ignore
          }
      ...
      res.setHeader('x-ticket-type-served', hasFinal ? 'final' : 'hold');
      ...
      if ((explicitType === 'final' || explicitType === 'hold') && strictType) {
        const wanted = explicitType === 'final' ? 'final e-ticket' : 'reserved ticket';
        return res.status(503).send(`Failed to generate ${wanted}`);
      }
