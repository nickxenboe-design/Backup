...
import { makePayment as makeEaglelinerPayment, printTickets as printEaglelinerTickets } from '../services/eagleliner/reservations.service.js';
import { getPricingSettings } from '../config/runtimeSettings.js';
import drizzleDb, { payments as paymentsTable, carts as cartsPgTable, tickets as ticketsPgTable } from '../db/drizzleClient.js';
...
const findEaglelinerReservationIdByPnr = async (pnr) => {
  ...
  try {
    ...
    const isEagle =
      data.source === 'eagleliner' ||
      data.provider === 'eagleliner' ||
      !!data.eaglelinerReservationId ||
      !!(data.eagleliner && data.eagleliner.reservationId);
    ...
    return { pnr: resolved, reservationId: String(reservationId), record: data };
  } catch (_) {
    return null;
  }
};
...
const buildEaglelinerTicketsHtml = ({ reservationId, tickets }) => {
    ...
    try {
          ...
          await drizzleDb
            ...
            .onConflictDoUpdate({
              ...
              set: {
                ...
                rawResponse: sql`COALESCE(${paymentsTable.rawResponse}, '{}'::jsonb) || ${record.rawResponse}`
              }
            });
        } catch (e) {
      ...
      for (const reference of refs) {
        ...
        try {
          ...
          try {
            let busbudCartId = null;
            const eagle = await findEaglelinerReservationIdByPnr(reference);
            if (eagle && eagle.reservationId) {
              const locked = await acquireProcessingLock(reference);
              if (!locked) {
                item.poll = { success: true, status: 'payment_processing', message: 'Already processing', provider: 'eagleliner' };
              } else {
                try {
                  const amountToPay = (typeof options.amount === 'number' && options.amount > 0)
                    ? options.amount
                    : (typeof inv.amount_residual === 'number' && inv.amount_residual > 0 ? inv.amount_residual : inv.amount_total);

                  const eaglePayRequestId = `payments-confirm-eagle-makepay-${Date.now()}-${Math.floor(Math.random() * 100000)}`;
                  logger.info('[payments.confirm] Eagleliner make_payment start', {
                    requestId: eaglePayRequestId,
                    at: new Date().toISOString(),
                    reference,
                    reservationId: eagle.reservationId,
                    amountReceived: amountToPay,
                    paymentMethod: 1,
                  });

                  const payRes = await makeEaglelinerPayment({
                    reservationId: eagle.reservationId,
                    amountReceived: amountToPay,
                    paymentMethod: 1,
                  });

                  logger.info('[payments.confirm] Eagleliner make_payment end', {
                    requestId: eaglePayRequestId,
                    at: new Date().toISOString(),
                    reference,
                    reservationId: eagle.reservationId,
                    success: Boolean(payRes && payRes.Success !== false),
                    error: payRes && (payRes.Error || payRes.error || payRes.message) ? String(payRes.Error || payRes.error || payRes.message) : null,
                    response: payRes || null,
                  });

                  const ok = Boolean(payRes && payRes.Success !== false);
                  if (!ok) {
                    const msg = (payRes && (payRes.Error || payRes.error || payRes.message))
                      ? String(payRes.Error || payRes.error || payRes.message)
                      : 'Eagleliner payment failed';
                    item.poll = {
                      success: false,
                      code: 'EAGLELINER_PAYMENT_FAILED',
                      error: msg,
                      provider: 'eagleliner',
                      reservationId: eagle.reservationId,
                    };
                    await markBookingFailedEverywhere({
                      ...
                      busbudCartId: null,
                      reason: msg,
                      meta: { source: 'payments.confirm', provider: 'eagleliner', reservationId: eagle.reservationId }
                    });
                    try {
                      await persistConfirmation({ reference, amount: amountToPay, status: 'eagleliner_failed', invoiceId, detail: { provider: 'eagleliner', reservationId: eagle.reservationId, response: payRes } });
                    } catch (_) {}
                  } else {
                    try {
                      await persistConfirmation({ reference, amount: amountToPay, status: 'paid', invoiceId, detail: { provider: 'eagleliner', reservationId: eagle.reservationId, response: payRes } });
                    } catch (_) {}
                    ...
                    try {
                      const db = await getFirestore();
                      await db.collection('carts').doc(String(reference)).set({
                        status: 'confirmed',
                        paid: true,
                        paymentStatus: 'paid',
                        eaglelinerReservationId: String(eagle.reservationId),
                        eaglelinerPayment: payRes || null,
                        updatedAt: new Date().toISOString(),
                      }, { merge: true });
                    } catch (_) {}

                    const currency = Array.isArray(inv.currency_id) ? (inv.currency_id[1] || 'USD') : 'USD';
                    try { await incrementTicketCounters(reference); } catch (_) {}
                    try { await markProcessed(reference); } catch (_) {}
                    try {
                      await sendPaymentWebhook({
                        event: 'payment.confirmed',
                        status: 'confirmed',
                        ...
                        currency,
                        confirmedAt: new Date().toISOString(),
                        metadata: { source: 'payments.confirm', provider: 'eagleliner', reservationId: eagle.reservationId }
                      });
                    } catch (_) {}

                    try {
                      const email = (() => {
                        const r = eagle && eagle.record ? eagle.record : null;
                        const candidate =
                          (r && r.eagleliner && r.eagleliner.contactInfo && (r.eagleliner.contactInfo.email || r.eagleliner.contactInfo.Email)) ||
                          (r && r.contactInfo && (r.contactInfo.email || r.contactInfo.Email)) ||
                          (r && r.purchaser && (r.purchaser.email || r.purchaser.Email)) ||
                          (r && r.customer && (r.customer.email || r.customer.Email)) ||
                          null;
                      ...
                      if (!email) {
                        logger.warn('[payments.confirm] Eagleliner email skipped (no purchaser email found)', { reference });
                      } else if (!process.env.SMTP_HOST || !process.env.SMTP_USERNAME || !process.env.SMTP_PASSWORD) {
                        logger.warn('[payments.confirm] Eagleliner email skipped (SMTP not configured)', { reference, to: email });
                      } else {
                        ...
                        // Use unified ticket PDF endpoint so Eagleliner renders with the same template as Busbud.
                        // IMPORTANT: use regen=1 for the attachment generation to avoid re-serving any previously cached legacy PDFs.
                        const url = `${apiBase}/api/ticket/pdf?pnr=${encodeURIComponent(String(reference))}&type=final&strict=1&paper=thermal48&download=1`;
                        const fetchUrl = `${apiBase}/api/ticket/pdf?pnr=${encodeURIComponent(String(reference))}&type=final&strict=1&paper=thermal48&regen=1`;

                        const pdfResponse = await axios.get(fetchUrl, {
                          responseType: 'arraybuffer',
                          validateStatus: () => true,
                        });
                        if (!pdfResponse || pdfResponse.status !== 200) {
                          throw new Error(`Failed to generate unified ticket PDF (status ${pdfResponse && pdfResponse.status != null ? pdfResponse.status : 'unknown'})`);
                        }
                        const buf = Buffer.from(pdfResponse.data || []);
                        if (!buf || !buf.length) {
                          throw new Error('Failed to generate unified ticket PDF (empty response)');
                        }
                        ...
                        try {
                          ...
                          logger.warn('[payments.confirm] Failed to persist Eagleliner ticket PDF to Postgres', { reference, error: e.message });
                        }

                        const subject = `Your Eagleliner ticket is ready - Ref ${reference}`;
                        const bodyHtml = `<div><h2>Hello,</h2><p>Your payment has been confirmed and your Eagleliner ticket is ready.</p><p><strong>Booking reference (PNR):</strong> ${escapeHtml(reference)}</p><p>Download your ticket PDF here: <a href="${escapeHtml(url)}">Download ticket</a></p></div>`;
                        await sendEmail({
                          to: email,
                          subject,
                          html: bodyHtml,
                          attachments: [
                            {
                              filename: `eticket-${String(reference)}.pdf`,
                              content: buf,
                      ...
                      logger.warn('[payments.confirm] Failed to send Eagleliner ticket email (non-blocking)', { reference, error: e.message });
                    }

                    item.poll = { success: true, status: 'confirmed', provider: 'eagleliner', reservationId: eagle.reservationId };
                  }
                } finally {
                  await releaseProcessingLock(reference);
                }
          ...
          if (item.poll && item.poll.success) {
            item.status = 'ok';
            if (item.poll.provider === 'eagleliner') {
