...
router.post('/', async (req, res) => {
  const { email } = req.body || {};
  if (!email) return res.status(400).json({ success: false, error: 'MISSING_EMAIL', message: 'Email is required' });
  try {
    ...
    return res.status(400).json({
      ...
      message: (err && err.message) ? err.message : 'Failed to create admin'
    });
  }
});
...
router.post('/register', async (req, res) => {
  try {
    ...
    try {
      await auth.createUser({ email, password, emailVerified: false, disabled: false });
    } catch (e) {
    ...
    if (code === 'auth/email-already-exists') {
      return res.status(409).json({ success: false, error: 'EMAIL_ALREADY_EXISTS', message: 'An admin account with this email already exists' });
    }
    ...
    return res.status(400).json({
      success: false,
      error: 'ADMIN_REGISTER_FAILED',
      message: (err && err.message) ? err.message : 'Admin registration failed'
    });
  }
});
