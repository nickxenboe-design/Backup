...
import { searchTripsByCities, normalizeEaglelinerTrip } from '../services/eagleliner/index.js';
...
router.get('/locations', async (req, res) => {
  ...
  try {
    ...
    if (!searchTerm) {
      ...
      const text = `
        SELECT city_id, country_code2, city_name, city_geohash, city_lat, city_lon
        FROM public.cities
...
router.get('/', async (req, res) => {
  ...
  const { origin, destination, date, returnDate, adults, children, seniors, students, currency, language, age, provider } = req.query;

  const resolvedProvider = String(provider || 'busbud').trim().toLowerCase();
  ...
  // When there are children, require ages in the query (Busbud only)
  if (options.children > 0 && resolvedProvider !== 'eagleliner') {
    if (!age) {
  ...
  try {
    ...
    if (resolvedProvider === 'eagleliner') {
      const passengerCounts = {
        adults: Math.max(0, Number(options.adults || 0)),
        children: Math.max(0, Number(options.children || 0)),
        seniors: Math.max(0, Number(options.seniors || 0)),
        students: Math.max(0, Number(options.students || 0)),
      };

      const passengersCount = Math.max(
        1,
        passengerCounts.adults + passengerCounts.children + passengerCounts.seniors + passengerCounts.students
      );

      const eagleTripsRaw = await searchTripsByCities({
        fromCity: origin,
        toCity: destination,
        departureDate: date,
      ...
      logger.info(`📦 [${requestId}] Eagleliner trips raw response normalized`, {
        rawTripsCount: Array.isArray(eagleTripsRaw) ? eagleTripsRaw.length : 0,
      });

      const trips = (eagleTripsRaw || []).map((t) =>
        normalizeEaglelinerTrip(t, {
      ...
      logger.info(`✅ [${requestId}] Eagleliner search completed successfully in ${responseTime}ms`, {
