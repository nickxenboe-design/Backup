...
import drizzleDb, { carts, payments } from '../db/drizzleClient.js';
import { and, desc, eq, gte, ilike, inArray, lte, or, sql } from 'drizzle-orm';
import { listPassengerTypes as listEaglelinerPassengerTypes } from '../services/eagleliner/passengerTypes.service.js';
...
function buildEaglePassengerTypeLabelMap(data) {
  try {
    const payload = data && typeof data === 'object' ? data : {};
    const rows = Array.isArray(payload.PassengerTypes)
      ? payload.PassengerTypes
      : (Array.isArray(payload.passengerTypes) ? payload.passengerTypes : []);

    const out = new Map();
    for (const r of rows) {
      if (!r || typeof r !== 'object') continue;
      const id = r.TypeID ?? r.typeId ?? r.id;
      const name = r.Name ?? r.name ?? r.Label ?? r.label;
...
function normalizeEaglePassengerTypeLabel(typeValue, typeLabelMap) {
  if (typeValue == null) return null;
  const raw = String(typeValue).trim();
  if (!raw) return null;

  // If a numeric ID (or a numeric-ish string), map it to a label.
  if (typeLabelMap && raw && /^\d+$/.test(raw) && typeLabelMap.has(raw)) {
    return typeLabelMap.get(raw);
  }

  // Some payloads already provide a string label like "Adult".
  if (typeLabelMap) {
    for (const [, label] of typeLabelMap.entries()) {
      if (label && String(label).trim().toLowerCase() === raw.toLowerCase()) {
        return label;
      }
    }
  }
...
function eagleTicketNumberFromAny(obj) {
  try {
    if (!obj || typeof obj !== 'object') return null;
    const v = obj.TicketNumber || obj.ticketNumber || obj.ticket_no || obj.ticketNo || null;
...
router.get('/sales-summary', async (req, res) => {
  try {
    ...
    let eaglePassengerTypeMap = new Map();
    try {
      const typesData = await listEaglelinerPassengerTypes();
      eaglePassengerTypeMap = buildEaglePassengerTypeLabelMap(typesData);
    } catch (_) {
      eaglePassengerTypeMap = new Map();
    }
    ...
    let cartMap = new Map();
    try {
      const refs = Array.from(new Set((paymentRows || []).map((r) => String(r.transactionRef || '').trim()).filter(Boolean)));
      if (refs.length) {
        const cartRows = await drizzleDb
          .select({
            cartId: carts.cartId,
            firestoreCartId: carts.firestoreCartId,
            passengerCount: carts.passengerCount,
            purchaser: carts.purchaser,
            passengers: carts.passengers,
            busbudResponse: carts.busbudResponse,
            origin: carts.origin,
            destination: carts.destination,
            departAt: carts.departAt,
          })
          .from(carts)
          .where(or(inArray(carts.cartId, refs), inArray(carts.firestoreCartId, refs)));

        cartMap = new Map();
        for (const c of cartRows || []) {
          const ids = [c.cartId, c.firestoreCartId].map((x) => String(x || '').trim()).filter(Boolean);
          for (const id of ids) {
            cartMap.set(id, c);
          }
        }
    ...
    for (const row of paymentRows) {
      ...
      const refKey = row.transactionRef ? String(row.transactionRef).trim() : '';
      const cart = refKey ? (cartMap.get(refKey) || null) : null;
      const cartPayload = cart && cart.busbudResponse ? cart.busbudResponse : null;
      const isEagleliner = !!(
        cartPayload &&
        typeof cartPayload === 'object' &&
        (
          cartPayload.provider === 'eagleliner' ||
          cartPayload.source === 'eagleliner' ||
          !!cartPayload.eaglelinerReservationId ||
          (cartPayload.eagleliner && cartPayload.eagleliner.reservationId)
        )
      );

      const n = isEagleliner
        ? (cart && cart.passengerCount != null ? (Number(cart.passengerCount) || 1) : 1)
        : 1;

      const revenue = Number(row.amount || 0);
      const currency = isEagleliner ? 'ZAR' : 'USD';
      const cost = 0;
      ...
      const operatorName = isEagleliner ? 'Eagleliner' : deriveOperatorFromPurchase(purchase);
...
router.get('/transactions', async (req, res) => {
  try {
    ...
    for (const row of paymentRows) {
      ...
      if (isEagleliner) {
        const purchaser = cart && cart.purchaser && typeof cart.purchaser === 'object' ? cart.purchaser : {};
        const paidByName = [purchaser.firstName || purchaser.first_name || '', purchaser.lastName || purchaser.last_name || ''].filter(Boolean).join(' ').trim();
        const paidByEmail = purchaser.email || purchaser.Email || null;
        const paidBy = paidByName || paidByEmail || null;

        const eagle = cartPayload && cartPayload.eagleliner && typeof cartPayload.eagleliner === 'object' ? cartPayload.eagleliner : {};
        const departure = (cart && cart.origin ? String(cart.origin) : (eagle.departureStopId != null ? String(eagle.departureStopId) : null)) || null;
        const destination = (cart && cart.destination ? String(cart.destination) : (eagle.destinationStopId != null ? String(eagle.destinationStopId) : null)) || null;
        const departureDate = eagle.departureDate || (cart && cart.departAt ? String(cart.departAt) : null) || null;

        const passengersList = Array.isArray(cart && cart.passengers) ? cart.passengers : (Array.isArray(eagle.passengerDetails) ? eagle.passengerDetails : []);
        const printedTickets = (eagle && Array.isArray(eagle.tickets))
          ? eagle.tickets
          : (cartPayload && cartPayload.eagleliner && Array.isArray(cartPayload.eagleliner.tickets) ? cartPayload.eagleliner.tickets : []);
        const firstPrintedTicketNo = printedTickets && printedTickets.length ? eagleTicketNumberFromAny(printedTickets[0] || null) : null;
        const ticketRef = cartPayload.eaglelinerReservationId || (eagle && eagle.reservationId) || null;
        ...
        if (!passengersList.length) {
          rows.push({
            ...
            ticket: firstPrintedTicketNo || ticketRef,
            name: null,
            departure,
            destination,
            departureDate,
          ...
          for (const p of passengersList) {
            ...
            const lastName = p?.Surname || p?.lastName || p?.last_name || '';
            const fullName = [firstName, lastName].filter(Boolean).join(' ').trim() || null;
            const passengerTicket =
              eagleTicketNumberFromAny(p) ||
              eagleTicketNumberFromAny(p?._eaglelinerTicket) ||
              (printedTickets && printedTickets.length ? eagleTicketNumberFromAny(printedTickets[passengerIdx] || null) : null) ||
              firstPrintedTicketNo ||
              ticketRef;
            const passengerTypeRaw = p?.Type || p?.type || p?.TypeID || p?.typeId || p?.PassengerType || p?.passengerType || (p?._eaglelinerTicket && (p._eaglelinerTicket.Type ?? p._eaglelinerTicket.type)) || null;
            const passengerType = normalizeEaglePassengerTypeLabel(passengerTypeRaw, eaglePassengerTypeMap);
            rows.push({
              paidOn,
              paidBy,
              bookedBy: row.bookedBy || paidBy || null,
