...
router.post('/select', async (req, res) => {
  ...
  try {
    ...
    if (typeof tripId === 'string' && tripId.startsWith('eagleliner:')) {
      return res.status(400).json({
        success: false,
        error: 'Eagleliner booking is not enabled yet. Please select a Busbud trip for now.',
        requestId,
    ...
    let cartResponse = await BusbudService.createCart(req.body?.currency || "USD", req.clientBranch || null);
    ...
    try {
      ...
      if (errType === 'CartNotSet') {
        ...
        const newCart = await BusbudService.createCart(req.body?.currency || "USD", req.clientBranch || null);
