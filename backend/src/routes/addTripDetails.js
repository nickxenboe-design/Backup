    ...
    try {
      if (firestoreCartId) {
        ...
        if (isAgentMode && agentEmail) {
          try {
            const now = new Date();
            const bookedBySafe = String(agentEmail || '').trim().toLowerCase();
            if (debug) {
              console.log('🧾 Upserting canonical Postgres cart booked_by', {
                cartId: String(firestoreCartId),
                firestoreCartId: String(firestoreCartId),
          ...
          try {
            ...
            if (debug) {
              console.log('🧾 Upserting awaiting_payment Postgres cart', {
                ...
                firestoreCartId: responseFirestoreCartId ? String(responseFirestoreCartId) : (paymentRef ? String(paymentRef) : null),
                status: 'awaiting_payment',
                bookedBy: bookedBySafe || null,
