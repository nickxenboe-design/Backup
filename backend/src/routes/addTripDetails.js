...
router.post('/frontend', async (req, res) => {
  try {
    ...
    try {
      ...
      if (debug) console.log('   - Using in-memory cart with ID:', existingCart.id);
    }
    ...
    const firestoreCartId = await ensureFirestoreCartId(actualCartId, req.clientBranch || null);
    ...
    try {
      if (firestoreCartId) {
        ...
        // Update PostgreSQL carts table with booked_by if agentEmail is available
        if (agentEmail) {
          try {
            await drizzleDb
              .update(cartsPgTable)
              .set({ bookedBy: String(agentEmail).trim().toLowerCase() })
              .where(eq(cartsPgTable.cartId, String(actualCartId)));
            console.log(`✅ Updated PostgreSQL cart booked_by to ${agentEmail} for cartId: ${actualCartId}`);
          } catch (updateError) {
            console.warn('⚠️ Failed to update PostgreSQL cart booked_by:', updateError.message);
          }
        }
      }
    } catch (_) {}
    ...
    if (req.body.hold === true || req.body.hold === 'true') {
      console.log('🔄 Hold is true, creating invoice...');
        ...
        if (pgCart) {
          cartData = {
            busbudCartId: pgCart.cartId,
            firestoreCartId: pgCart.firestoreCartId,
...
console.log('💵 Price data for invoice:', {
  ...
  itemAmounts: retailPrice && Array.isArray(retailPrice.items) ? retailPrice.items.map(it => it.amount) : []
        ...
        if (!cartExpiryDate) {
          try {
            let ttlFirestoreCartId = BusbudService.firestoreCartId || null;
        ...
        try {
          ...
          try {
            ...
            await drizzleDb
              .insert(cartsPgTable)
              .values({
                cartId: String(actualCartId),
                firestoreCartId: responseFirestoreCartId ? String(responseFirestoreCartId) : null,
                status: 'awaiting_payment',
