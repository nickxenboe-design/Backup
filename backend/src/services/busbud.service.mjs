    ...
    this.axios.interceptors.request.use(
      config => {
        ...
        const logEntry = {
          ...
          data: isProduction ? undefined : (config.data || {}),
          headers: (() => {
      ...
      error => {
        ...
        const errorLog = {
          ...
          response: response ? {
            ...
            data: isProduction ? undefined : response.data
          } : null
        };
...
/**
   ...
   */
  async getCart(cartId, locale = 'en-ca', currency = 'USD', options = null) {
    try {
      if (!cartId) throw new Error('cartId is required');

      const opts = options && typeof options === 'object' ? options : {};
      const bypassCache = Boolean(opts.bypassCache || opts.noCache || opts.fresh);
  ...
  async updatePurchaser(cartId, purchaser, locale = 'en-ca') {
    ...
    try {
      ...
      logger.debug('Purchaser Details', { purchaser });
  ...
  async updateTripPassengers(cartId, tripId, { locale, currency, savePassengerQuestionAnswers }, passengers, ticketTypes) {
    ...
    try {
      ...
      const stripNonExistentQuestion = (questionKey) => {
        try {
          const bad = String(questionKey || '').trim();
          if (!bad) return false;
          let removed = false;
          if (!requestBody || !Array.isArray(requestBody.passengers)) return false;
          for (const p of requestBody.passengers) {
            const list = Array.isArray(p && p.answers) ? p.answers : [];
            const next = [];
            for (const a of list) {
              const k = a && (a.question_key || a.questionKey || a.key);
              if (k != null && String(k).trim().toLowerCase() === bad.toLowerCase()) {
                removed = true;
                continue;
              }
              next.push(a);
      ...
      const tryUpdateTripPassengers = async () => {
        const startTime = Date.now();
        const response = await axios.put(
      ...
      while (true) {
        try {
          attempt += 1;
          ({ response, duration } = await tryUpdateTripPassengers());
          break;
        } catch (error) {
          const status = error && error.response ? error.response.status : null;
          const details = error && error.response && error.response.data && error.response.data.error
            ? (error.response.data.error.details || error.response.data.error.message || '')
            : '';

          // Busbud may reject answers for questions that do not exist for this operator/trip.
          // Example: "Passenger answer received for non-existent question dob".
          const m = typeof details === 'string'
            ? details.match(/non-existent\s+question\s+([a-zA-Z0-9_\-]+)/i)
            : null;
          const badKey = m && m[1] ? String(m[1]).trim() : '';

          if (status === 400 && badKey && attempt < 3) {
            const removed = stripNonExistentQuestion(badKey);
            if (removed) {
              logger.warn(`[${requestId}] Busbud rejected non-existent question; retrying without it`, {
                cartId: cartId ? `${cartId.substring(0, 4)}...${cartId.slice(-4)}` : 'none',
                tripId: tripId ? `${tripId.substring(0, 4)}...${tripId.slice(-4)}` : 'none',
                badKey,
                attempt
