...
function App() {
  ...
  const handleAcknowledgePriceUpdate = useCallback(() => {
    ...
    setCurrentBooking((prev) => {
      ...
      const outboundPrice = isAggregatedRoundTrip ? finalTotal / 2 : finalTotal;
      return {
        ...prev,
        outbound: {
          ...prev.outbound,
  ...
  const handleSearch = useCallback(async (query: SearchQuery, options?: { isReturnLegSearch?: boolean }) => {
    ...
    try {
      ...
      // Replace Google GenAI call with local API call
      const response = await searchTrips({ ...query, provider: 'busbud' });
      ...
      if (response && response.length >= 0) {
        ...
        const shouldFetchEagleliner = (query?.tripType === 'one-way') && !options?.isReturnLegSearch;
        if (shouldFetchEagleliner) {
          void (async () => {
            try {
              const eagleTrips = await searchTrips({
                ...query,
                provider: 'eagleliner',
              ...
              if (Array.isArray(eagleTrips) && eagleTrips.length > 0) {
                setRoutes((prev) => {
                  const existingIds = new Set((prev || []).map((t) => t.id));
                  const merged = [...(prev || [])];
                  for (const t of eagleTrips) {
        ...
        // Ensure the search query has the searchId for trip selection (non-blocking)
        if (response.length > 0 && currentQuery) {
          console.log(' Starting searchId extraction from trips...');
          console.log(' Response length:', response.length);
          console.log(' First trip data:', {
            id: response[0]?.id,
            search_id: response[0]?.search_id,
            searchId: response[0]?.search_id
          });

          // Extract searchId from any trip (not just the first one)
          let searchId = response.find(trip => trip.search_id)?.search_id;
          console.log(' Found searchId from trip.search_id:', searchId);

          // If not found in trip.search_id, try to extract from trip data structure
          if (!searchId) {
            const firstTrip = response[0];
            console.log(' Trying to extract from first trip data structure...');
            ...
            if (firstTrip.id) {
              try {
                ...
                console.log(' Extracted searchId from trip data structure:', searchId);
              } catch (e) {
                console.warn(' Could not decode trip ID as base64 JSON:', e);
              }
            }
          }

          if (searchId) {
            console.log(' Updating search query with searchId:', searchId);
            console.log(' Found searchId in trip:', response.find(trip => trip.search_id));
            setCurrentQuery(prev => prev ? { ...prev, searchId } : null);
          } else {
            console.log(' No searchId found - trip selection will work without it (non-blocking)');
          }
        }
      } else {
        setError("No trips found for your search.");
      }
    } catch (e) {
      console.error(e);
      const mapped = mapTripErrorToUserMessage({
  ...
  const handleConfirmBooking = (opts?: { eaglelinerPassengerCounts?: Record<string, number>; estimatedTotal?: number }) => {
      try {
        if (opts && currentQuery) {
          if (opts.eaglelinerPassengerCounts) {
            setCurrentQuery((prev) => {
              if (!prev) return prev;
              return {
                ...prev,
                eaglelinerPassengerCounts: opts.eaglelinerPassengerCounts,
              };
            });
            try {
              saveCartData({ eaglelinerPassengerCounts: opts.eaglelinerPassengerCounts });
            } catch {}
          }

          if (typeof opts.estimatedTotal === 'number' && Number.isFinite(opts.estimatedTotal)) {
            try {
              const currency = currentBooking?.outbound?.currency || 'USD';
              saveCartData({ quotedTotal: opts.estimatedTotal, quotedCurrency: currency });
            } catch {}
          }
        }
      } catch {}
    ...
    if (currentBooking.outbound && currentQuery) {
      try {
        ...
        const bookingResult = await submitBooking({
          ...
          selectedSeatIds: (details as any).selectedSeatIds,
          estimatedTotal: (details as any).estimatedTotal,
          ...
          try {
            ...
            const fallbackQuoted = (() => {
              try {
                ...
                return isAggregatedRoundTrip ? outbound * 2 : outbound;
              } catch {
                return null;
              }
            })();
        ...
        <div className="relative container mx-auto px-4 md:px-6 z-10">
          <div className="max-w-3xl mx-auto animate-fade-in">
            <h1 className="text-2xl md:text-4xl font-extrabold text-[#652D8E] dark:text-purple-300 tracking-tight">
              Bus tickets across Zimbabwe & South Africa
            </h1>
        ...
        return (
          <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900">
            <div className="text-sm text-gray-600 dark:text-gray-300">
              ...
              {agentAuthError ? <div className="mt-2 text-red-600 dark:text-red-400">{agentAuthError}</div> : null}
            </div>
          </div>
        );
      }
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50 dark:bg-gray-900">
        ...
        <ErrorModal
          ...
          variant="info"
          primaryActionLabel="Accept & continue"
