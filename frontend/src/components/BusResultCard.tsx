...
const BusResultCard: React.FC<BusResultCardProps> = ({ route, searchQuery, onTripSelected, disabled = false, deferApiCall = false, isSelectingInbound = false, onError, badges }) => {
    ...
    const passengerCount = Math.max(1, (searchQuery?.passengers?.adults || 0) + (searchQuery?.passengers?.children || 0) + (searchQuery?.passengers?.seniors || 0) + (searchQuery?.passengers?.students || 0));
            ...
            <div className="p-3 md:p-4 grid grid-cols-12 gap-3 items-center">
                ...
                <div className="col-span-12 md:col-span-3 md:border-l border-gray-200/80 dark:border-gray-700 md:pl-3 flex flex-row md:flex-col items-center justify-between md:justify-center md:items-end text-right h-full">
                    ...
                    <button
                        ...
                        <span>{(disabled || isSelecting) ? 'Selecting...' : 'Continue'}</span>
                    </button>
                </div>
            </div>
        </div>
