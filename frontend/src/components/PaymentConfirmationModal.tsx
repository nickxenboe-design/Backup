...
import { CheckCircleIcon, ArrowRightIcon, UserCircleIcon, AtSymbolIcon, CreditCardIcon, PayPalIcon, GooglePayIcon, StoreIcon, CalendarIcon, LocationIcon, TicketIcon, ShareIcon, DownloadIcon, SpinnerIcon, ClockIcon, PhoneIcon } from './icons';
...
const PassengerDetailsCard: React.FC<{ passenger: Passenger, index: number }> = ({ passenger, index }) => (
    <div className="border-t border-gray-200 dark:border-gray-700 pt-2 first:pt-0 first:border-t-0">
        ...
        <div className="mt-1.5 grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-1">
            ...
            {passenger.idNumber ? (
                <PassengerDetailRow icon={<CreditCardIcon />} label={(passenger.idType || 'ID').replace('_', ' ')} value={passenger.idNumber} />
            ) : null}
            {passenger.emergencyContactNumber ? (
                <PassengerDetailRow icon={<PhoneIcon />} label="Next of kin" value={passenger.emergencyContactNumber} />
            ) : null}
        </div>
    </div>
);

const generatePnrPdfBlob = (pnr: string, expiryText?: string, supportEmail?: string, isEaglelinerTrip?: boolean) => {
    const doc = new jsPDF();

    doc.setFontSize(16);
    doc.text(isEaglelinerTrip ? 'Booking reference' : 'Reservation (PNR)', 10, 15);

    doc.setFontSize(12);
    doc.text(`${isEaglelinerTrip ? 'Booking reference' : 'Reservation Code'}: ${pnr}`, 10, 30);

    doc.setFontSize(10);
    if (isEaglelinerTrip) {
        doc.text('Eagleliner payment notice', 10, 42);

        doc.setFontSize(11);
        doc.text(
            'This Eagleliner booking must be paid within 15 minutes or it will be automatically terminated and you will need to restart the booking.',
            10,
            52,
            { maxWidth: 190 }
        );
        doc.text('If you are not ready to pay now, please make your booking again a few hours before departure.', 10, 74, { maxWidth: 190 });
        doc.text(`To make payment, contact National Tickets: ${supportEmail || 'support@nationaltickets.co.za'}`, 10, 96, { maxWidth: 190 });
    } else {
...
const PaymentConfirmationModal: React.FC<PaymentConfirmationModalProps> = ({ isOpen, onClose, onConfirm, showInfo, booking, query, details, pnr, maxWidth }) => {
    ...
    const isEaglelinerTrip = useMemo(() => {
        const tripId = String((details as any)?.tripId || (booking as any)?.outbound?.id || '').trim();
        if (tripId && tripId.startsWith('eagleliner:')) return true;
        const raw = (booking as any)?.outbound && (booking as any)?.outbound?._eagleliner;
        return Boolean(raw);
    }, [booking, details]);
    ...
    useEffect(() => {
        ...
        fetchHoldTicket();
    }, [isOpen, details.paymentMethod, isEaglelinerTrip]);
    ...
    const handleSharePnr = async () => {
        ...
        const blob = generatePnrPdfBlob(pnr, expiryText, supportEmail, isEaglelinerTrip);

        const fileName = `${isEaglelinerTrip ? 'booking' : 'reservation'}-${pnr}.pdf`;
        ...
        if (canShareFile && typeof nav.share === 'function') {
            try {
                await nav.share({
                    files: [file],
                    title: isEaglelinerTrip ? 'Booking reference' : 'Reservation Code (PNR)',
                    text: isEaglelinerTrip ? 'Booking reference PDF attached.' : 'Reservation code PDF attached.',
                });
                return;
            } catch {}
        }
        ...
        if (typeof navigator !== 'undefined' && navigator.clipboard) {
            try {
                const text = isEaglelinerTrip ? `Booking reference: ${pnr}` : `Reservation Code (PNR): ${pnr}`;
    ...
    const handleDownloadPnr = () => {
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = `${isEaglelinerTrip ? 'booking' : 'reservation'}-${pnr}.pdf`;
        a.click();
        URL.revokeObjectURL(url);
    };
    ...
    const handleDownloadHoldTicket = () => {
        ...
        const run = async () => {
            ...
            try {
                ...
                const response = await fetch(`/api/ticket/hold/pdf/${encoded}?regen=1&v=${Date.now()}`);
                if (!response.ok) {
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-1 sm:p-2" role="dialog" aria-modal="true">
            <div ref={modalRef} className={`w-full ${modalWidthClass} h-[90vh] max-h-[90vh]`}>
                <div className="bg-white dark:bg-gray-800 p-3 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 h-full overflow-hidden flex flex-col min-h-0">
                    {isEaglelinerTrip && (
                        <div className="mb-2 rounded-lg border border-amber-200 dark:border-amber-900/40 bg-amber-50 dark:bg-amber-950/25 px-3 py-2">
                            <div className="flex items-start gap-2">
                                <ClockIcon className="h-4 w-4 mt-0.5 text-amber-700 dark:text-amber-300" aria-hidden="true" />
                                <div className="min-w-0">
                                    <div className="text-[11px] font-bold text-amber-900 dark:text-amber-200">
                                        Eagleliner payment notice
                                    </div>
                                    <div className="mt-1 text-[11px] text-amber-900/90 dark:text-amber-100/90">
                                        This Eagleliner booking must be paid within <span className="font-semibold">15 minutes</span> or it will be automatically terminated and you will need to restart the booking.
                                    </div>
                                    <div className="mt-1 text-[11px] text-amber-900/90 dark:text-amber-100/90">
                                        If you are not ready to pay now, please make your booking again <span className="font-semibold">a few hours before departure</span>.
                                    </div>
                                    <div className="mt-2 text-[11px] text-amber-900/90 dark:text-amber-100/90">
                                        To make payment, contact <span className="font-semibold">National Tickets</span>:
                                    </div>
                                    <div className="mt-1 flex flex-wrap items-center gap-x-4 gap-y-1 text-[11px]">
                                        ...
                                        {supportWhatsappDigits ? (
                                            ...
                                            >
                                                ...
                                                <span className="font-semibold">{supportWhatsapp || 'WhatsApp'}</span>
                                            </a>
                                        ) : null}
                                    </div>
                                </div>
                    ...
                    <p className="text-[11px] text-gray-600 dark:text-gray-300 mb-2">
                        {details.paymentMethod === 'In-Store'
                            ? (isEaglelinerTrip
                                ? 'Your booking is ready. Use the booking reference below and follow the Eagleliner payment notice. Click Finish when you are done.'
                    ...
                    {details.paymentMethod === 'In-Store' && !isEaglelinerTrip && (holdTicketLoading || holdTicketPdfLoading) && (
                    ...
                    {details.paymentMethod === 'In-Store' && !isEaglelinerTrip && holdTicketError && <p className="text-[11px] text-red-600 dark:text-red-400">{holdTicketError}</p>}
                    {details.paymentMethod === 'In-Store' && !isEaglelinerTrip && holdTicket && !holdTicketLoading && !holdTicketError && (
                                ...
                                <div className="flex items-center gap-1.5">
                                    <button
                                        ...
                                        onClick={() => {
                                            ...
                                            if (!pdfPnr) return;
                                            const encoded = encodeURIComponent(String(pdfPnr));
                                            window.open(`/api/ticket/hold/pdf/${encoded}?regen=1&v=${Date.now()}#zoom=page-width`, '_blank', 'noopener,noreferrer');
                                        }}
