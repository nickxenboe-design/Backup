...
const ViewTicketsPage: React.FC = () => {
  ...
  const pdfEndpoint = useMemo(() => {
    if (!pnr) return null;
    return buildUrlWithAgent(`${API_BASE_URL}/ticket/pdf`, { pnr });
  }, [pnr]);

  const eticketEndpoint = useMemo(() => {
    if (!pnr) return null;
    return buildUrlWithAgent(`${API_BASE_URL}/ticket/eticket/pdf`, { pnr });
  }, [pnr]);

  const holdEndpoint = useMemo(() => {
    if (!pnr) return null;
    return buildUrlWithAgent(`${API_BASE_URL}/ticket/hold/pdf/${encodeURIComponent(pnr)}`, { regen: '1', v: String(Date.now()) });
  }, [pnr]);

  const downloadEndpoint = useMemo(() => {
    if (!downloadPnr) return null;
    return buildUrlWithAgent(`${API_BASE_URL}/ticket/pdf`, { pnr: downloadPnr, download: '1' });
  }, [downloadPnr]);
  ...
  const downloadFromEndpoint = async (endpoint: string, fallbackFilename: string) => {
    try {
      ...
      if (!res.ok) {
        throw new Error(`Download failed from ${endpoint} (status ${res.status})`);
      }
  ...
  useEffect(() => {
    if (!pdfEndpoint && !eticketEndpoint && !holdEndpoint) return;
    ...
    const loadPdf = async () => {
      try {
        ...
        const endpoints = (() => {
          if (ticketType === 'final') return [eticketEndpoint].filter(Boolean) as string[];
          if (ticketType === 'hold') return [holdEndpoint].filter(Boolean) as string[];
          return [pdfEndpoint, eticketEndpoint, holdEndpoint].filter(Boolean) as string[];
        })();
        ...
        for (const endpoint of endpoints) {
          ...
          try {
            ...
            if (res.status === 204 || buf.byteLength === 0) {
              throw new Error(
                `No ticket PDF available yet from ${endpoint} (status ${res.status}, content-type ${contentType || 'unknown'})${snippet ? `: ${snippet}` : ''}`
              );
            }

            if (!res.ok) {
              throw new Error(
                `Failed to load ticket PDF from ${endpoint} (status ${res.status}, content-type ${contentType || 'unknown'})${snippet ? `: ${snippet}` : ''}`
              );
            }
            ...
            if (!looksLikePdf) {
              throw new Error(
                `Unexpected response when loading ticket PDF from ${endpoint} (status ${res.status}, content-type ${contentType || 'unknown'})${snippet ? `: ${snippet}` : ''}`
              );
            }
            ...
            if (controller.signal.aborted) return;
            lastError = e instanceof Error ? e : new Error(e?.message || 'Failed to load ticket PDF');
          }
        }
  ...
  }, [pdfEndpoint, eticketEndpoint, holdEndpoint, ticketType]);
