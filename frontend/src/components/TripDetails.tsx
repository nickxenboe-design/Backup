...
const ExpandableStationText: React.FC<{ text: string; className?: string }> = ({ text, className }) => {
    ...
    return (
        <button
            ...
            className={`${className} ${expanded ? 'whitespace-normal break-words' : 'truncate'} bg-transparent p-0 text-left cursor-pointer focus:outline-none`}
            title={safeText}
...
const ItineraryLeg: React.FC<{ title: string, departureDate: string, arrivalDate: string, route: BusRoute }> = ({ title, departureDate, arrivalDate, route }) => (
    <div className="bg-white dark:bg-gray-800 p-2 rounded-xl shadow-md border border-gray-200 dark:border-gray-700 text-[11px]">
        <h2 className="text-xs font-bold text-[#652D8E] dark:text-purple-300 mb-0.5">
            <span className="inline-block max-w-[46%] align-middle">
                <ExpandableStationText text={route.origin} className="inline-block max-w-full align-middle truncate" />
            </span>
            <span className="mx-1 align-middle">to</span>
            <span className="inline-block max-w-[46%] align-middle">
                <ExpandableStationText text={route.destination} className="inline-block max-w-full align-middle truncate" />
            </span>
        </h2>
        ...
        <div className="space-y-3">
            ...
            <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <InfoRow icon={<DepartureIcon className="h-3 w-3"/>} label="From" value={<ExpandableStationText text={route.origin} className="block w-full truncate" />} />
            ...
            {/* Arrival */}
             <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <InfoRow icon={<ArrivalIcon className="h-3 w-3"/>} label="To" value={<ExpandableStationText text={route.destination} className="block w-full truncate" />} />
