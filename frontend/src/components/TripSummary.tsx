...
const TripSummary: React.FC<TripSummaryProps> = ({ booking, query, compact, onChangeRequested }) => {
    ...
    const totalPassengers = Math.max(1, (query.passengers.adults || 0) + (query.passengers.children || 0) + ((query.passengers as any).seniors || 0) + ((query.passengers as any).students || 0));
    ...
    const outboundSeatMeta = {
        available: typeof (outbound as any)?.availableSeats === 'number' ? (outbound as any).availableSeats : undefined,
        reserved: typeof (outbound as any)?.reservedSeats === 'number' ? (outbound as any).reservedSeats : undefined,
        occupied: typeof (outbound as any)?.occupiedSeats === 'number' ? (outbound as any).occupiedSeats : undefined,
    };
    ...
    return (
        <div className={`relative ${compact ? 'space-y-2.5' : 'space-y-4'}`}>
            ...
            <div className="pt-3 border-t border-gray-200 dark:border-gray-700">
                ...
                {(outboundSeatMeta.available != null || outboundSeatMeta.reserved != null || outboundSeatMeta.occupied != null) && (
                    <div className="mt-2 grid grid-cols-3 gap-2 text-[11px]">
                        <div className="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900/50 px-2 py-1.5">
                            <div className="text-[9px] font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">Available</div>
                            <div className="text-sm font-bold text-gray-800 dark:text-gray-100">{outboundSeatMeta.available ?? '—'}</div>
                        </div>
                        <div className="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900/50 px-2 py-1.5">
                            <div className="text-[9px] font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">Reserved</div>
                            <div className="text-sm font-bold text-gray-800 dark:text-gray-100">{outboundSeatMeta.reserved ?? '—'}</div>
                        </div>
                        <div className="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900/50 px-2 py-1.5">
                            <div className="text-[9px] font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">Occupied</div>
                            <div className="text-sm font-bold text-gray-800 dark:text-gray-100">{outboundSeatMeta.occupied ?? '—'}</div>
                        </div>
                    </div>
                )}
