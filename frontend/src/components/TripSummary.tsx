...
const TripSummary: React.FC<TripSummaryProps> = ({ booking, query, compact, onChangeRequested }) => {
    ...
    let outboundBasePrice = outbound.price || 0;
    let inboundBasePrice = inbound
        ? (inbound.price || 0)
        : 0;

    if (hasAggregatedRoundTrip) {
        const totalFromRoute = outbound.price || 0;

        if (legsTotal > 0) {
            outboundBasePrice = outboundLegPrice;
            inboundBasePrice = inboundLegPrice;
        } else if (Number.isFinite(outboundLegPrice) && outboundLegPrice > 0 && totalFromRoute > 0) {
            outboundBasePrice = outboundLegPrice;
            inboundBasePrice = Math.max(0, totalFromRoute - outboundLegPrice);
        } else if (Number.isFinite(inboundLegPrice) && inboundLegPrice > 0 && totalFromRoute > 0) {
            inboundBasePrice = inboundLegPrice;
            outboundBasePrice = Math.max(0, totalFromRoute - inboundLegPrice);
        } else if (totalFromRoute > 0) {
            outboundBasePrice = totalFromRoute / 2;
            inboundBasePrice = totalFromRoute / 2;
        }
    }
    ...
    const outboundFare = computePassengerFareBreakdown({ ...outbound, price: outboundBasePrice }, query);
    const inboundFare = inboundToShow ? computePassengerFareBreakdown({ ...inboundToShow, price: inboundBasePrice }, query) : null;
