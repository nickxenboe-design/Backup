...
const BusSearchBar: React.FC<BusSearchBarProps> = ({ onSearch, loading, initialQuery }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const showValidationError = (message: string) => {
      setErrorModalTitle('Check your details');
    ...
    if (selectedDate < today) {
      showValidationError('Please choose a departure date that is today or later.');
      return;
    }
    ...
    if (tripType === 'round-trip' && returnDate) {
      ...
      if (returnDateObj <= selectedDate) {
        showValidationError('Return date must be at least one day after the departure date.');
        return;
      }
    }
