...
const PassengerInfo: React.FC<PassengerInfoProps> = ({ booking, query, onBack, onReview }) => {
    ...
    const handleDocumentUpload = async (index: number, file: File | null) => {
        ...
        if (file.size > 5 * 1024 * 1024) {
            setUploadDocError('File is too large. Please upload a file up to 5MB.');
    ...
    const validate = () => {
        ...
        passengers.forEach((p, i) => {
            ...
            if (p.dob) {
                ...
                newErrors.passengers[i].dob = 'Date of birth is required for child passengers.';
                isValid = false;
            }

            if (p.emergencyContactName.trim() || p.emergencyContactNumber.trim()) {
                if (!p.emergencyContactName.trim()) {
                    newErrors.passengers[i].emergencyContactName = 'Emergency contact name is required.';
                    isValid = false;
                }
                    ...
                    <div className="lg:col-span-2 space-y-4">
                        ...
                        <div className="bg-white dark:bg-gray-800 p-3 rounded-2xl shadow-md border border-gray-200 dark:border-gray-700">
                            <div className="flex items-center justify-between gap-2 mb-1.5 border-b border-gray-200 dark:border-gray-700 pb-1.5">
                                ...
                                {(() => {
                                    const ok = passengers.every(p => p.firstName.trim() && p.lastName.trim() && p.idType && p.idNumber.trim() && (p.type !== 'child' || (p.dob && p.dob.trim())));
                                    ...
                                    <div key={index} className="border-t border-gray-200/80 dark:border-gray-700/80 pt-3 first:pt-0 first:border-t-0">
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                            ...
                                            <div className="sm:col-span-2">
                                                <details className="mt-1" open={passenger.type === 'child'}>
                                                    ...
                                                    <div className="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-2">
                                                        ...
                                                        <div className="sm:col-span-2 flex flex-col gap-1 text-[11px] mt-0.5">
                                                            ...
                                                            <div className="inline-flex items-center gap-1 text-[10px] text-gray-600 dark:text-gray-300">
                                                                <LockClosedIcon className="h-3.5 w-3.5 text-gray-400" />
                                                                Secure document handling. Only shared with the operator if required.
                                                            </div>
                        ...
                        <div className="bg-white dark:bg-gray-800 p-3 rounded-2xl shadow-md border border-gray-200 dark:border-gray-700">
                             ...
                             <div className="mt-3 pt-3 border-t border-gray-200/80 dark:border-gray-700/80">
                                <label htmlFor="c-optInMarketing" className="flex items-center cursor-pointer select-none group">
                                    ...
                                    <span className="ml-2 text-xs text-gray-700 dark:text-gray-300">
                                        Receive marketing emails and special offers. No spam. Unsubscribe anytime.
                                    </span>
                                </label>
                            </div>
                        </div>
