...
const PassengerInfo: React.FC<PassengerInfoProps> = ({ booking, query, onBack, onReview }) => {
    ...
    const handlePassengerChange = (index: number, field: keyof Omit<PassengerWithAnswers, 'type' | 'questionAnswers'>, value: string) => {
                                ...
                                {passengers.map((passenger, index) => (
                                    <div key={index} className="border-t border-gray-200/80 dark:border-gray-700/80 pt-3 first:pt-0 first:border-t-0">
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                            ...
                                            />
                                            {isAllowedKey('id_type') && isRequiredKey('id_type') && (
                                                <CustomSelectField
                                                    id={`p${index}-idType`}
                                                    label="ID Type"
                                            ...
                                            {isAllowedKey('id_number') && isRequiredKey('id_number') && (
                                                <FormField
                                                    id={`p${index}-idNumber`}
                                                    label="ID Number"
                                                    icon={<CreditCardIcon className="h-4 w-4 text-gray-400" />}
                                            ...
                                            {getDynamicQuestionKeys().filter((k) => isRequiredKey(k)).map((k) => (
                                                <FormField
                                                    key={`p${index}-${k}`}
                                                    id={`p${index}-${k}`}
                                            ...
                                            {(passenger.type === 'child') || (isAllowedKey('dob') && isRequiredKey('dob')) ? (
                                                <DateFormField
                                                    id={`p${index}-dob`}
                                            ...
                                            {isAllowedKey('gender') && isRequiredKey('gender') && (
                                                <CustomSelectField
                                                    id={`p${index}-gender`}
                                                    label="Gender"
                                                    icon={<UserCircleIcon className="h-4 w-4 text-gray-400" />}
                                            ...
                                            {isAllowedKey('nationality') && isRequiredKey('nationality') && (
