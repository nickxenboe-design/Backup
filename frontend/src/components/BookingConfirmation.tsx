...
import { isAgentModeActive } from '@/utils/agentHeaders';
...
const BookingConfirmation: React.FC<BookingConfirmationProps> = ({ booking, query, details, onNewBooking, onViewTickets, pnr }) => {
    ...
    const agentMode = isAgentModeActive();
    ...
    }, [isInStorePayment, isEaglelinerTrip, reservationRef]);
    ...
    const buildShareText = () => {
        ...
        return [
            ...
            reservationRef ? `${isEaglelinerTrip ? 'Booking reference' : 'Reservation Ref'}: ${reservationRef}` : undefined,
        ].filter(Boolean).join('\n');
    };
    ...
    const handleDownloadReservationPdf = () => {
        ...
        const doc = new jsPDF();
        doc.setFontSize(14);
        doc.text(isEaglelinerTrip ? 'Booking Confirmation' : 'Reservation Confirmation', 10, 14);
        doc.setFontSize(11);
        doc.text(`${isEaglelinerTrip ? 'Booking reference' : 'Reservation Ref'}: ${reservationRef}`, 10, 28);
        ...
        if (isInStorePayment) {
            if (isEaglelinerTrip) {
                doc.setFontSize(11);
                doc.text('Eagleliner payment notice', 10, 92);
                doc.setFontSize(11);
                doc.text(
                ...
                doc.text('If you are not ready to pay now, please make your booking again a few hours before departure.', 10, 126, { maxWidth: 190 });
                doc.text(`To make payment, contact National Tickets: ${supportEmail}`, 10, 148, { maxWidth: 190 });
            } else {
        ...
        doc.save(`${isEaglelinerTrip ? 'booking' : 'reservation'}-${reservationRef}.pdf`);
    };
    ...
    return (
        <div className="max-w-xl mx-auto animate-fade-in py-6">
                ...
                <div className="mt-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/30 p-3">
                    <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0">
                            <div className="text-[10px] uppercase tracking-wider text-gray-500 dark:text-gray-400 font-semibold">
                                {isEaglelinerTrip ? 'Booking reference' : 'Reservation reference'}
                            </div>
                        ...
                        <button
                            ...
                            aria-label={isEaglelinerTrip ? 'Copy booking reference' : 'Copy reservation reference'}
                            title={isEaglelinerTrip ? 'Copy booking reference' : 'Copy reservation reference'}
                        >
            ...
            {isInStorePayment ? (
                isEaglelinerTrip ? (
                    <div className="mt-3 rounded-2xl border border-amber-200 dark:border-amber-900/40 bg-amber-50 dark:bg-amber-950/25 p-3">
                        <div className="flex items-start gap-2">
                            <ClockIcon className="h-4 w-4 mt-0.5 text-amber-700 dark:text-amber-300" aria-hidden="true" />
                            <div className="min-w-0">
                                <div className="text-[11px] font-bold text-amber-900 dark:text-amber-200">Eagleliner payment notice</div>
                                ...
                                <div className="mt-1 flex flex-wrap items-center gap-x-4 gap-y-1 text-[11px]">
                                    <a href={`mailto:${supportEmail}`} className="inline-flex items-center gap-1.5 text-amber-800 dark:text-amber-200 hover:underline">
                                        <AtSymbolIcon className="h-3.5 w-3.5" aria-hidden="true" />
                                        <span className="font-semibold">{supportEmail}</span>
                                    </a>
                                    {supportPhone ? (
                                        <a href={`tel:${supportPhone}`} className="inline-flex items-center gap-1.5 text-amber-800 dark:text-amber-200 hover:underline">
                                            <PhoneIcon className="h-3.5 w-3.5" aria-hidden="true" />
                                            <span className="font-semibold">{supportPhone}</span>
                                        </a>
                                    ) : null}
                                    {supportWhatsappDigits ? (
                                        <a href={`https://wa.me/${supportWhatsappDigits}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-amber-800 dark:text-amber-200 hover:underline">
            ...
            <div className="mt-3 rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm p-3">
                ...
                <div className="mt-2 grid grid-cols-1 gap-2 text-[12px]">
                    ...
                    <div className="flex items-center gap-2 text-gray-700 dark:text-gray-200">
                        ...
                        <span>{isInStorePayment ? (isEaglelinerTrip ? 'Pay within 15 minutes' : 'Pay in-store') : 'Tickets ready'}</span>
                    </div>
                    <div className="flex items-center gap-2 text-gray-700 dark:text-gray-200">
            ...
            <div className="mt-4 text-center flex flex-col sm:flex-row items-center justify-center gap-2">
                ...
                >
                    ...
                    <span>
                        {agentMode
                            ? (isEaglelinerTrip ? 'View booking' : 'View reservation')
                            : (isInStorePayment ? (isEaglelinerTrip ? 'View booking' : 'View reservation') : (onViewTickets ? 'View ticket(s)' : 'Get Ticket(s)'))}
                    </span>
                </button>
