...
const Results: React.FC<ResultsProps> = ({ routes, loading, isRefetching, error, query, booking, onSelectRoute, onSearch, onEditSearch, onResetOutbound, onTripError }) => {
  ...
  const handleSelectingChange = (selecting: boolean, route: BusRoute) => {
    if (selecting) {
      setSelectingRouteId(route.id);
      return;
    }
    setSelectingRouteId((prev) => (prev === route.id ? null : prev));
  };
                            ...
                            {filteredAndSortedRoutes.map((route, index) => (
                                <div key={route.id} id={`route-${route.id}`}>
                                    <BusResultCard
                                        ...
                                        disabled={Boolean(selectingRouteId && selectingRouteId !== route.id)}
                                    />
                                </div>
                            ))}
                        </div>
