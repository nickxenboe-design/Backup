...
const AgentDashboard: React.FC = () => {
  ...
  const handlePnrSearch = async () => {
    ...
    try {
      ...
      // Fallback: resolve Eagleliner internal PNR -> upstream reservationId
      try {
        const headers: any = {
          ...getAgentHeaders(),
        };

        const endpoint = `${API_BASE}/api/eagleliner/agent/lookup?pnr=${encodeURIComponent(cleaned)}`;
        ...
        const data: any = await res.json().catch(() => null);
        if (res.ok && data && data.ok && data.pnr) {
          setPnrResult({
            reference: String(data.pnr),
            cartId: String(data.pnr),
            source: 'eagleliner',
            status: (data.record && (data.record.status || data.record.state)) ? (data.record.status || data.record.state) : 'awaiting_payment',
            eaglelinerReservationId: data.reservationId ? String(data.reservationId) : null,
        ...
        setPnrResult(null);
        setPnrSearchMessage('No booking found for that PNR.');
      } catch (e: any) {
        setPnrResult(null);
        setPnrSearchMessage(e?.message || 'No booking found for that PNR.');
      }
    } finally {
                  ...
                  >
                    ...
                    {pnrResult && (
                      <div>
                        ...
                        {pnrResult.source === 'eagleliner' && pnrResult.eaglelinerReservationId ? (
                          <div style={{ marginTop: 4 }}>
                            <div>
                              <strong>Eagleliner reservation ID:</strong>{' '}
                              {String(pnrResult.eaglelinerReservationId)}
                            </div>
                          </div>
                        ) : null}
                      </div>
                ...
                <tbody>
                  {recentBookingsLoading && recentBookings.length === 0 ? (
                    ...
                    recentBookingsSlice.map((row: any, idx: number) => {
                      ...
                      const downloadUrl = `${API_BASE}/api/ticket/pdf?pnr=${encodeURIComponent(String(pnr))}&download=1${paxCount > 1 ? '&zip=1' : ''}${bookingType === 'paid' ? '&type=final&strict=1' : ''}`;
                      const amountNumber =
                          ...
                          <button
                            ...
                            onClick={() => {
                              if (typeof window !== 'undefined') {
                                ...
                                const isEagle = String(raw || '').includes('/api/eagleliner/');
