...
class BusbudService {
  ...
  async createCart(currency = 'USD') {
    try {
      ...
      try {
        const firestoreCartId = await ensureFirestoreCartId(response.data.id);
...
/**
  ...
  async _savePassengerSnapshotToPostgres(cartId, requestId) {
    try {
      ...
      const passengers = Array.isArray(passengerDetails.passengers)
  ...
  _processCharges(charges) {
    ...
    try {
      ...
      if (Array.isArray(processedCharges.items)) {
        processedCharges.items = processedCharges.items.map(item => {
          if (typeof item.amount !== 'number') return item;

          const originalAmountCents = item.amount;
          const currency = item.currency || charges.currency || 'USD';
        ...
        try {
          ...
          const documentData = {
            busbudResponse: {
              ...
              original_charges: response.data, // Keep original for reference
              adjusted_charges: processedCharges, // Store processed charges
              lastUpdated: new Date().toISOString()
            },
            updatedAt: new Date().toISOString(),
            pricing_metadata: {
  ...
  async updatePurchaserDetails(cartId, purchaser, firestoreCartId = null) {
    ...
    try {
      ...
      // Save purchaser details to Firestore after successful API call
      try {
        if (!this.db) {
          this.db = await getFirestore();
        }

        const resolvedId = await ensureFirestoreCartId(cartId);
        this.firestoreCartId = resolvedId;
        const documentId = resolvedId;
        
        firestoreLogger.info(`Saving purchaser details to Firestore for user ${documentId}`, {
          cartId,
          firestoreCartId: this.firestoreCartId,
          usingfirestoreCartId: !!this.firestoreCartId
        });
        ...
        if (!doc.exists) {
          const createData = {
            ...
            status: 'active',
            metadata: {
              createdBy: 'updatePurchaserDetails',
              lastUpdatedBy: 'updatePurchaserDetails'
            }
          };
          await cartRef.set(createData);
          await upsertCartFromFirestore(documentId, createData);
        } else {
          const updateData = {
            purchaserDetails: {
              ...purchaser,
            ...
            updatedAt: new Date().toISOString(),
            'metadata.lastUpdatedBy': 'updatePurchaserDetails'
          };
          await cartRef.update(updateData);
          await upsertCartFromFirestore(documentId, updateData);
        }
        
        firestoreLogger.info(`Successfully saved purchaser details to Firestore for user ${documentId}`);
      } catch (firestoreError) {
        // Log the error but don't fail the operation
        firestoreLogger.error('Failed to save purchaser details to Firestore', {
          cartId,
          firestoreCartId,
          error: firestoreError.message,
          stack: firestoreError.stack
      ...
      try {
        await this._savePassengerSnapshotToPostgres(cartId, requestId);
      } catch (snapshotError) {
