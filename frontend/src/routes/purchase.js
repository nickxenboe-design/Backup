...
async function pollBusbudPurchaseStatus(purchaseId, purchaseUuid, requestId) {
  const maxAttempts = Number(process.env.BUSBUD_PURCHASE_STATUS_MAX_ATTEMPTS || 10);
  const intervalMs = Number(process.env.BUSBUD_PURCHASE_STATUS_INTERVAL_MS || 3000);
...
async function savePurchaseToPostgres(purchaseId, purchaseUuid, purchaseDetails, extraMeta, pollOutcome, requestId) {
  try {
    ...
    const status = purchaseDetails.status || 'completed';
  ...
  try {
    ...
    try {
      ...
      if (isCompleted) {
        ...
        console.log(`[${requestId}] 📥 Final purchase details retrieved successfully:`, JSON.stringify(purchaseDetails, null, 2));
      } else {
        console.log(`[${requestId}] ⚠️ Purchase did not reach completed state during polling (outcome=${pollOutcome}).`);
        purchaseDetails = statusSnapshot || {
          status: 'failed',
          message: 'Purchase did not complete during status polling'
        };
      }
    } catch (detailsError) {
    ...
    console.log(`[${requestId}] Final purchase state:`, JSON.stringify(purchaseDetails, null, 2));
    ...
    try {
      ...
      const ticketDoc = {
        ...
        firestoreTicket,
        busbudPurchase: purchaseDetails || null,
      };
    ...
    logger.info(`✅ [${requestId}] Purchase completed successfully in ${responseTime}ms`);
...
router.post('/', async (req, res) => {
  ...
  logger.info(`💰 [${requestId}] Purchase completion request initiated`, {
    ...
    requestBody: req.body, // Log actual payload
    headers: req.headers
  });
  ...
  try {
    ...
    const result = await completePurchase(purchaseId, purchaseUuid, requestId, startTime);
