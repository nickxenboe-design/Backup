...
const usePostgresFirstForEticket = process.env.ETICKET_USE_POSTGRES_FIRST === 'true';
 ...
 const getBusbudReferenceFromPurchase = (purchase) => {
   ...
   add(booking.reference_number);
   add(booking.referenceNumber);
   add(booking.booking_reference);
   add(booking.bookingReference);
   add(booking.reference_no);
   add(booking.referenceNo);
   add(purchase.booking_reference);
   add(purchase.bookingReference);
   add(purchase.reference);
   add(purchase.reference_number);
   add(purchase.referenceNumber);
   add(purchase.reference_no);
   add(purchase.referenceNo);

   if (Array.isArray(purchase.items)) {
     for (const it of purchase.items) {
       if (!it || typeof it !== 'object') continue;
       const fields = it.fields || {};
       add(fields.booking_reference);
       add(fields.bookingReference);
       add(fields.reference);
       add(fields.reference_number);
       add(fields.referenceNumber);
       add(it.booking_reference);
       add(it.bookingReference);
       add(it.reference);
     }
   }
 ...
 const getBusbudBookingReference = (cart) => {
   const sources = [
     cart && cart.passengerDetails && cart.passengerDetails.completePurchase,
     cart && cart.passengerDetails && cart.passengerDetails.busbudResponse,
     cart && cart.busbudPurchase,
     cart && cart.completePurchase,
     cart && cart.purchase,
     cart && cart.busbudResponse
   ];
   for (const src of sources) {
     const ref = getBusbudReferenceFromPurchase(src);
     if (ref) return ref;
   }
   return null;
 };
...
const loadTicketCartFromPostgres = async (pnr, requestId) => {
  try {
    ...
    const ticketRef = getBusbudReferenceFromPurchase(purchase);
    let origin = null;
...
router.post(
  ...
  try {
    ...
    const refNo = pnr;
    const ticketNo = (() => {
      const bbRef = getBusbudBookingReference(cart);
      return bbRef || cart.ticketNo || cart.ticket_no || cart.bookingId || cart.booking_id || cart.purchaseId || cart.purchase_id || (cart.purchase && (cart.purchase.id || cart.purchase.uuid)) || refNo;
    })();
