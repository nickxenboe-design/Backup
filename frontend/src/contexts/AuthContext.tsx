...
import { getAgentHeaders, setAgentHeaders, setAgentModeActive } from '../utils/agentHeaders';
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const isAgentScope = () => {
    ...
    try {
      ...
      return String((h as any)['x-agent-mode'] || '').toLowerCase() === 'true';
    } catch {
      return false;
    }
  };
  ...
  const signIn = async (email: string, password: string, options?: { remember?: boolean }) => {
    ...
    try {
      ...
      if (typeof options?.remember === 'boolean') {
        payload.remember = options.remember;
      }
  ...
  const signUp = async (params: {
    ...
    try {
      ...
      if (!res.ok) {
        const msg = await extractErrorMessage(res, 'Signup failed');
