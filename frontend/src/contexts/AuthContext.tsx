...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const buildUrls = () => {
    ...
    const LOGIN_URL = agent
      ? joinApiUrl(API_BASE, '/api/v1/auth/agent/login')
      : ((import.meta as any).env?.VITE_AUTH_LOGIN_URL || joinApiUrl(API_BASE, '/api/v1/auth/login'));
    const SIGNUP_URL = (import.meta as any).env?.VITE_AUTH_SIGNUP_URL || joinApiUrl(API_BASE, '/api/v1/auth/signup');
    const LOGOUT_URL = agent
      ? joinApiUrl(API_BASE, '/api/v1/auth/agent/logout')
      : ((import.meta as any).env?.VITE_AUTH_LOGOUT_URL || joinApiUrl(API_BASE, '/api/v1/auth/logout'));
    const ME_URL = agent
      ? joinApiUrl(API_BASE, '/api/v1/auth/agent/me')
      : ((import.meta as any).env?.VITE_AUTH_ME_URL || joinApiUrl(API_BASE, '/api/v1/auth/me'));
    const RESET_URL = (import.meta as any).env?.VITE_AUTH_RESET_PASSWORD_URL || joinApiUrl(API_BASE, '/api/v1/auth/forgot-password');
