...
export interface CartSessionData {
  ...
  eaglelinerPassengerCounts?: Record<string, number>;
  passengerQuestions?: {
    required: string[];
    optional: string[];
    all: string[];
...
export const mapTrip = (trip: any, searchContext?: Partial<Pick<BusRoute, "search_id">>): BusRoute => {
  ...
  try {
    ...
    const result = {
      ...mapped,
      raw: (trip && (trip._eagleliner || trip._raw)) ? (trip._eagleliner || trip._raw) : trip,
...
// ---------------------
export const searchLocations = async (query: string, limit = 10): Promise<LocationSuggestion[]> => {
...
export const searchTrips = async (searchQuery: SearchQuery): Promise<BusRoute[]> => {
  ...
  try {
    const provider = String((searchQuery as any)?.provider || 'busbud');
    ...
    const run = (async () => {
      try {
        ...
        const params = new URLSearchParams({
          ...
          ...(Array.isArray(passengers.childrenAges) && passengers.childrenAges.length > 0
            ? { age: passengers.childrenAges.join(',') }
            : {}),
  ...
  try {
    ...
    if (cartId && tripId) {
      ...
      try {
        ...
        const quotedReturn = isAggregatedRoundTrip
          ? quotedOutbound
          : (returnTrip && typeof returnTrip.price === 'number' && Number.isFinite(returnTrip.price)
              ? returnTrip.price
              : 0);
        const quotedTotal = quotedOutbound + quotedReturn;
...
export const submitBooking = async ({
  ...
  searchQuery: SearchQuery;
    ...
    console.log('📋 [Booking] Selected seats:', selectedSeatIds);
    console.log('📋 [Booking] Trip ID:', tripId);
    console.log('📋 [Booking] Search query:', searchQuery);

  try {
    const isEaglelinerTrip = typeof tripId === 'string' && tripId.startsWith('eagleliner:');

    if (isEaglelinerTrip) {
      const decodeEaglelinerTripId = (value: string) => {
        const parts = String(value || '').split(':');
        // eagleliner:{TripID}:{DepartureStopID}:{DestinationStopID}:{DepartureTimeIso}
        return {
          tripId: parts[1] || '',
          departureStopId: parts[2] || '',
          destinationStopId: parts[3] || '',
        };
      };

      const parsed = decodeEaglelinerTripId(tripId);
      const paxCount = Math.max(1, Array.isArray(passengers) ? passengers.length : 0);

      const seatIds = Array.isArray(selectedSeatIds)
        ? selectedSeatIds.map((s) => String(s || '').trim()).filter(Boolean)
        : [];
      if (seatIds.length !== paxCount) {
        throw new Error(`Please select exactly ${paxCount} seat(s).`);
      }

      const passengerDetails = (Array.isArray(passengers) ? passengers : []).map((p, idx) => {
        // Eagleliner upstream accepts PassengerDetails forwarded as-is; we use a conservative field set.
        // If upstream requires more fields, we can extend later.
        const title = (() => {
          const raw = String((p as any)?.title || '').trim();
          return raw || 'Mr';
        })();

        const gender = (() => {
          const raw = String((p as any)?.gender || '').trim().toLowerCase();
          if (!raw) return '';
        ...
        // For Eagleliner, we treat passenger emergency contact phone as the primary telephone to send upstream.
        const telephone = String((p as any)?.emergencyContactNumber || contactInfo?.phone || '').trim();

        const idNumber = String((p as any)?.idNumber || '').trim();
        const idType = String((p as any)?.idType || '').trim().toLowerCase();

        const withInfant = Boolean((p as any)?.withInfant);

        const paxType = (() => {
          const t = Number((p as any)?.eaglelinerTypeId);
          if (Number.isFinite(t) && t > 0) return t;
          const rawType = String((p as any)?.type || '').trim().toLowerCase();
          if (rawType === 'child') return 2;
          return 1;
        })();
        ...
        const base: any = {
          ...
          Type: paxType,
          Title: title,
          Firstname: String((p as any)?.firstName || '').trim(),
          Surname: String((p as any)?.lastName || '').trim(),
          Gender: gender,
          Telephone: telephone,
          Seat: seatId,
          WithInfant: withInfant,
          // These infant fields are stored for internal use only; backend sanitizes upstream payload.
          InfantFirstName: String((p as any)?.infantFirstName || '').trim(),
          InfantLastName: String((p as any)?.infantLastName || '').trim(),
        };

        if (idNumber) {
          if (idType === 'passport') base.PassportNo = idNumber;
          else base.IDNumber = idNumber;
        }
      ...
      const reservationPayload = {
        tripId: Number(parsed.tripId || 0),
        departureStopId: Number(parsed.departureStopId || 0),
        destinationStopId: Number(parsed.destinationStopId || 0),
        departureDate: String(searchQuery?.departureDate || ''),
        passengers: paxCount,
        reservationTime: 300,
        passengerDetails,
        selectedSeatIds: seatIds,
        estimatedTotal: typeof estimatedTotal === 'number' ? estimatedTotal : undefined,
        contactInfo,
      };

      const res = await fetch(`${API_BASE_URL}/eagleliner/trips/reserve_seats`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          ...getAgentHeaders(),
      ...
      if (!reservationId) {
        throw new Error('Reservation created but no ReservationID returned');
      }

      const internalPnr = data?.Meta?.pnr || data?.Meta?.PNR || null;

      return {
        success: true,
        bookingId: String(internalPnr || reservationId),
        pnr: String(internalPnr || reservationId),
        message: data?.Message || data?.message || 'Reservation created',
      };
    }
    ...
    const payload = {
      ...
      passengers: transformedPassengers.map((passenger, index) => {
        ...
        const allowedKeys = (() => {
          try {
            const raw = cartData && cartData.passengerQuestions && Array.isArray(cartData.passengerQuestions.all)
              ? cartData.passengerQuestions.all
              : null;
        ...
        const answers = allowedKeys
          ? rawAnswers.filter((a) => a && a.question_key && allowedKeys.has(String(a.question_key).trim().toLowerCase()))
          : rawAnswers;
