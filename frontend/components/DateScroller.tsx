...
const DateScroller: React.FC<DateScrollerProps> = ({ selectedDate, onDateSelect, loading }) => {
    ...
    return (
        <div className="relative">
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md border border-gray-200/80 dark:border-gray-700 p-2">
                <div className="flex items-center justify-between">
                    <button
                        onClick={handlePrev}
                        disabled={isPrevDisabled || loading}
                        className="p-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                        aria-label="Previous week"
                    >
                        <ChevronLeftIcon className="h-6 w-6 text-[#652D8E] dark:text-purple-300" />
                    </button>
                    
                    <div className="flex-grow grid grid-cols-7 gap-1 text-center">
                    ...
                    >
                        <ChevronRightIcon className="h-6 w-6 text-[#652D8E] dark:text-purple-300" />
                    </button>
                </div>
            </div>
        </div>
